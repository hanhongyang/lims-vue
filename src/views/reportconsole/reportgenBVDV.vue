        ...
        <div class="report-container">
          ...
          <table class="sample-table">
            ...
            <tr>
              ...
              <td colspan="4" class="colspan-cell" style="padding:10px 0">BVDV抗原检测
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <th colspan="2">判定<br>Conclusion</th>
            ...
            <tr>
              ...
              <td colspan="4" class="colspan-cell" style="padding:10px 0">{{ feedReportBase.sjph
              }}
              </td>
            </tr>
          </table>
          <div class="xfqm">
            <div class="ttxf">编制:<img :src="signTypes.编制人" class="qm" v-if="feedReportBase.editSign"></div>
            <div class="ttxf">审核:<img :src="signTypes.审核人" class="qm" v-if="feedReportBase.checkSign"></div>
            <div class="ttxf">批准:<img :src="signTypes.批准人" class="qm" v-if="feedReportBase.approveSign"></div>
          </div>
        ...
        <div class="report-container">
          ...
          <table class="sample-table" style=" font-size: 11px;">
            ...
            <tr v-for="(item, index) in combinedPagedList[0]" :key="index">
              ...
              <td>{{ item.mnh }}</td>
              <td>{{ item.bw }}</td>
              <td :style="{ color: item.pdjg === '阳性' || item.pdjg === '+' ? 'red' : 'inherit' }">
                {{ item.sn }}
              </td>
              <td :style="{ color: item.pdjg === '阳性' || item.pdjg === '+' ? 'red' : 'inherit' }">
                {{ item.pdjg }}
...
export default {
  ...
  mounted() {
    ...
    if (this.status == 0) {
      ...
      Reportsl(newAr).then(res => {
        ...
        res.data.opBloodJHReportSampleVo.bvdvkyList.forEach((item) => {
  ...
  computed: {
    ...
    combinedPagedList() {
      // 1. 合并数据：阳性在前，阴性在后
      const combinedList = this.negativeList;
