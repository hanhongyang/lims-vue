<template>
    <div id="printId" v-loading="loading">
        <div class="sta-main">
            <div class="PageStyle">
                <div class="biaotihdflex">
                    <div class="font15">报告编号：{{ feedReportBase.reportNo }}</div>
                </div>
                ...
                <div class="report-container">
                    ...
                    <table class="sample-table">
                        ...
                        <tr>
                            ...
                            <th colspan="2">检验日期</th>
                            <td colspan="2">{{ feedReportBase.testTime }}</td>

                        </tr>

                        <tr>
                            <th colspan="2">检测类别</th>
                            <td colspan="4" class="colspan-cell" style="padding:10px 0">{{ feedReportBase.testType }}
                            </td>
                        </tr>
                        <tr>
        ...
        <!-- 阳性第一页 -->
        <div class="sta-main" v-if="positivePagedList.length > 0">
            <div class="PageStyle">
                <div class="report-container">
                    <div class="report-header">
                        <div class="report-info">
                    ...
                    <table class="sample-table" style="font-size: 11px;">
                        <tr>
                            <td colspan="8" style="text-align:left">结核分枝杆菌PPD检测结果阳性牛号如下：</td>
                        </tr>
                        <tr v-for="(row, rowIndex) in positivePagedList[0]" :key="'positive-row-' + rowIndex">
                            <td v-for="(item, colIndex) in row" :key="'positive-col-' + colIndex">
                                {{ item }}
                            </td>
                            <!-- 如果是阳性数据的最后一行且数据不足8个，在剩余单元格中显示结束提示 -->
                            <td v-if="isLastPositiveRow(0, rowIndex) && row.length < 8" :colspan="8 - row.length"
                                style="text-align: center; background: #f0f0f0;">
                                检测结果内容结束
                            </td>
                            <!-- 如果不是最后一行或数据已满，用空单元格填充 -->
                            <td v-else v-for="n in (8 - row.length)" :key="'positive-empty-' + n">&nbsp;</td>
                        </tr>
                        <!-- 如果最后一行正好8个数据，新起一行显示结束提示 -->
                        <tr v-if="shouldShowNewEndRow('positive', 0)">
                            <td colspan="8" style="text-align: center; background: #f0f0f0;">
                                检测结果内容结束
                            </td>
                        </tr>
                    </table>
        ...
        <!-- 阳性后续页面 -->
        <template v-for="(page, pageIndex) in positivePagedList.slice(1)">
            <div class="sta-main" :key="'positive-page-' + pageIndex">
                <div class="PageStyle">
                    <div class="report-container" style="margin:0">
                        ...
                        <table class="sample-table" style="font-size: 11px;">
                            ...
                            <tr v-for="(row, rowIndex) in page" :key="'positive-row-' + rowIndex">
                                <td v-for="(item, colIndex) in row" :key="'positive-col-' + colIndex">
                                    {{ item }}
                                </td>
                                <!-- 如果是阳性数据的最后一行且数据不足8个，在剩余单元格中显示结束提示 -->
                                <td v-if="isLastPositiveRow(pageIndex + 1, rowIndex) && row.length < 8"
                                    :colspan="8 - row.length" style="text-align: center; background: #f0f0f0;">
                                    检测结果内容结束
                                </td>
                                <!-- 如果不是最后一行或数据已满，用空单元格填充 -->
                                <td v-else v-for="n in (8 - row.length)" :key="'positive-empty-' + n">&nbsp;</td>
                            ...
                            <!-- 如果最后一行正好8个数据，新起一行显示结束提示 -->
                            <tr v-if="shouldShowNewEndRow('positive', pageIndex + 1)">
                                <td colspan="8" style="text-align: center; background: #f0f0f0;">
                                    检测结果内容结束
                                </td>
                            </tr>
        ...
        <!-- 阴性第一页 -->
        <div class="sta-main" v-if="negativePagedList.length > 0">
            <div class="PageStyle">
                <div class="report-container">

                    <table class="sample-table" style="font-size: 11px;">
                        <tr>
                            <td colspan="8" style="text-align:left">结核分枝杆菌PPD检测结果阴性牛号如下：</td>
                        </tr>
                        <tr v-for="(row, rowIndex) in negativePagedList[0]" :key="'negative-row-' + rowIndex">
                            <td v-for="(item, colIndex) in row" :key="'negative-col-' + colIndex">
                                {{ item }}
                            </td>
                            <!-- 如果是阴性数据的最后一行且数据不足8个，在剩余单元格中显示结束提示 -->
                            <td v-if="isLastNegativeRow(0, rowIndex) && row.length < 8" :colspan="8 - row.length"
                                style="text-align: center; background: #f0f0f0;">
                                检测结果内容结束
                            </td>
                            <!-- 如果不是最后一行或数据已满，用空单元格填充 -->
                            <td v-else v-for="n in (8 - row.length)" :key="'negative-empty-' + n">&nbsp;</td>
                        </tr>
                        <!-- 如果最后一行正好8个数据，新起一行显示结束提示 -->
                        <tr v-if="shouldShowNewEndRow('negative', 0)">
                            <td colspan="8" style="text-align: center; background: #f0f0f0;">
                                检测结果内容结束
                            </td>
                        </tr>
        ...
        <!-- 阴性后续页面 -->
        <template v-for="(page, pageIndex) in negativePagedList.slice(1)">
            <div class="sta-main" :key="'negative-page-' + pageIndex">
                <div class="PageStyle">
                    <div class="report-container" style="margin:0">
                        <div class="report-header">
                            <div class="report-info">
                        ...
                        <table class="sample-table" style="font-size: 11px;">
                            ...
                            <tr v-for="(row, rowIndex) in page" :key="'negative-row-' + rowIndex">
                                <td v-for="(item, colIndex) in row" :key="'negative-col-' + colIndex">
                                    {{ item }}
                                </td>
                                <!-- 如果是阴性数据的最后一行且数据不足8个，在剩余单元格中显示结束提示 -->
                                <td v-if="isLastNegativeRow(pageIndex + 1, rowIndex) && row.length < 8"
                                    :colspan="8 - row.length" style="text-align: center; background: #f0f0f0;">
                                    检测结果内容结束
                                </td>
                                <!-- 如果不是最后一行或数据已满，用空单元格填充 -->
                            ...
                            <!-- 如果最后一行正好8个数据，新起一行显示结束提示 -->
                            <tr v-if="shouldShowNewEndRow('negative', pageIndex + 1)">
                                <td colspan="8" style="text-align: center; background: #f0f0f0;">
                                    检测结果内容结束
                                </td>
                            </tr>
...
import {
    Reportsl, Reportslbc, zzhhx, zzhhxbc, jzcommit, savePdfzy, ckbg, emailList,
    sendpdfzy,
...
export default {
    ...
    computed: {
        ...
        negativePagedList() {
            ...
            const firstPageData = this.negativeList.slice(0, 328);
            pages.push(chunkArray(firstPageData, 8));

            const remainingData = this.negativeList.slice(328);
            for (let i = 0; i < remainingData.length; i += 344) {
                const pageData = remainingData.slice(i, i + 344);
                pages.push(chunkArray(pageData, 8));
            }
        ...
        positivePagedList() {
            ...
            const firstPageData = this.positiveList.slice(0, 328);
            pages.push(chunkArray(firstPageData, 8));

            const remainingData = this.positiveList.slice(328);
            for (let i = 0; i < remainingData.length; i += 344) {
                const pageData = remainingData.slice(i, i + 344);
                pages.push(chunkArray(pageData, 8));
            }
        ...
        // 总页数计算（包括阴性页数和阳性页数）
        totalPages() {
            return 1 + this.negativePagedList.length + this.positivePagedList.length;
        },
        // 新增：格式化报告日期
        formattedReportDate() {
    ...
    methods: {
        ...
        /** 判断是否是阴性数据的最后一行 */
        isLastNegativeRow(pageIndex, rowIndex) {
            // 如果有阳性数据，阴性数据的所有行都不是最后一行


            const currentPage = this.negativePagedList[pageIndex];
            // 当前页的最后一行，并且是阴性数据的最后一页
            return rowIndex === currentPage.length - 1 && pageIndex === this.negativePagedList.length - 1;
        },

        /** 判断是否是阳性数据的最后一行 */
        isLastPositiveRow(pageIndex, rowIndex) {
            const currentPage = this.positivePagedList[pageIndex];
            // 当前页的最后一行，并且是阳性数据的最后一页
            return rowIndex === currentPage.length - 1 && pageIndex === this.positivePagedList.length - 1;
        },
        /** 判断是否需要新起一行显示结束提示 */
        shouldShowNewEndRow(type, pageIndex) {
            if (type === 'negative') {
                // 如果有阳性数据，阴性数据不需要显示结束行
                // if (this.positivePagedList.length > 0) return false;

                const currentPage = this.negativePagedList[pageIndex];
                const isLastPage = pageIndex === this.negativePagedList.length - 1;

                if (isLastPage && currentPage.length > 0) {
                    const lastRow = currentPage[currentPage.length - 1];
                    // 如果最后一行正好有8个数据，需要新起一行
                    return lastRow.length === 8;
                }
                return false;
            } else if (type === 'positive') {
                const currentPage = this.positivePagedList[pageIndex];
                const isLastPage = pageIndex === this.positivePagedList.length - 1;
        ...
        /** 计算需要合并的列数 */
        getRemainingColspan(pageData) {
            if (!pageData || pageData.length === 0) return 8;

            // 获取最后一行的数据数量
            const lastRow = pageData[pageData.length - 1];
            const lastRowLength = lastRow ? lastRow.length : 0;

            // 如果最后一行有8个数据，说明是完整的，需要新的一行显示结束
            if (lastRowLength === 8) {
                return 8;
            } else {
                // 否则合并剩余的单元格
                return 8 - lastRowLength;
            }
        },
        /** 导出单页 */
        pdfMap() {
        ...
        getpdf(stalength, mpddfcont) {
            ...
            html2canvas($(".sta-main")[mpddfcont], {
                ...
                if (mpddfcont == stalength - 1) {
                    ...
                    // const newCs = {
                    //     file: pdfBlob,
                    //     emailDTO: [{ "opJczxBloodReportBaseId": JSON.parse(this.$route.query.opJczxBloodReportBaseId), "feedEntrustOrderSampleId": JSON.parse(this.$route.query.feedEntrustOrderSampleId) }]
