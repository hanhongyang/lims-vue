                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="10" style="padding: 0; border: 0;">
                                <table cellpadding="0" cellspacing="0" width="100%" style="table-layout: fixed;">
                                    <tr>
                                        <td style="width: 16.65%;">AFB₁标准液浓度（μg/kg）</td>
                                        <td style="width: 13.33%;">0</td>
                                        <td style="width: 13.33%;">1.5</td>
                                        <td style="width: 13.33%;">5.0</td>
                                        <td style="width: 13.33%;">15.0</td>
                                        <td style="width: 13.33%;">50.0</td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td colspan="8" class="regression-row">
                                R=<el-input v-model="Details.bzqxfcjxgxs" style="width: 600px;"
                                ...
                                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>待测液中AFB₁含量C（ng/ml）</td>
                                            <td colspan="2">样品中AFB₁含量（μg/kg）</td>
                                            <td colspan="2">鲜样中AFB₁含量（μg/kg）</td>
                                            <td>操作</td>
                                        </tr>
                                        ...
                                        <template v-for="(item, index) in Details.infoList">
                                            <tr :key="`row-${index}`" class="sample-split"
                                                ...
                                                <td>
                                                    <el-input v-model="item.c" @input="bringCtoSample(item)"
...
import { feedwatertask, feedwatertaskUp, listTask, saveCheck, csfhy, retest, testSubmit, usersign, backToSubmit } from "@/api/hyd";
import electronic from '@/views/Feedproject/electronic.vue';
import csfhyMixintwoYX from "@/views/Feedproject/csfhyMixintwoYX";

import Decimal from 'decimal.js'
export default {
    name: "JJ53",
    mixins: [csfhyMixintwoYX],
    ...
    computed: {
        ...
        formulaTipsList() {
            ...
            (this.Details.infoList || []).forEach((row, idx) => {
                const rowEmpty = [];
                if (!row.c) rowEmpty.push('待测液中AFB₁含量');
    ...
    mounted() {
        ...
        if (this.$route.params.taskInfo) {
            this.dqID()
            this.invbillId = this.$route.params.CCinvbillId;
    ...
    methods: {
        ...
        validateEmpty() {
            ...
            /* ---- 逐行样品字段 ---- */
            (this.Details.infoList || []).forEach((row, idx) => {
                if (!row.c) empty.push(`第${idx + 1}行待测液中AFB₁含量`);
        ...
        computeAFB1(item) {
            ...
            try {
                const C = new Decimal(item.c);        // 待测液中AFB₁含量 (ng/mL)
                ...
                const finalResult = new Decimal(formattedResult).lessThan(1.5) ? '<1.5' : formattedResult;

                // 保存计算结果到字段
                this.$set(item, 'ypzxxhl', finalResult);
                console.log('计算AFB₁含量，输入项:', item, '计算结果:', finalResult);
                return finalResult;
            } catch (error) {
                console.error('计算AFB₁含量错误:', error);
                this.$set(item, 'ypzxxhl', '');
                return '';
            }
        },

        updateAFB1Values(item) {

            // 重新计算AFB₁含量并保存到对应字段
            // this.computeAFB1(item);
            // 计算鲜样含量
            //this.calculateFreshSampleForItem(item);
            // 1. 同步计算AFB₁含量 (computeAFB1 内部会设置 item.ypzxxhl)
            // this.computeAFB1(item);

            // 2. 【核心修复】：在 $nextTick 中调用 Mixin 流程，解决异步依赖问题
            this.$nextTick(() => {
                // 负数校验已在 computeAFB1 内部执行

                // 调用我们覆盖后的逻辑 (calculateFreshSampleForItem)
                // this.calculateFreshSampleForItem(item);
            });
        },
        ...
        calculateFreshSampleForItem(item) {
            ...
            // 2. 如果主要结果存在，直接查找初水分并调用计算（绕过 Mixin 的 item.average 检查）
            let csfValue = null;
        ...
        calculateSingleFreshSample(item, csfValue) {
            ...
            try {
                // JJ53 是单样检测，鲜样含量依赖样品中含量 (ypzxxhl)，而不是平均值 (average)
                const ypzxxhlValue = item.ypzxxhl;

                // 1. 检查 ypzxxhl 是否有效（跳过空值和 <检出限）
                if (ypzxxhlValue === null || ypzxxhlValue === undefined || ypzxxhlValue === '' || String(ypzxxhlValue).startsWith('<')) {
                    // 如果是 '<1.5' 等检出限，鲜样结果也应显示检出限或为空
                    // this.$set(item, 'xyzhl', ypzxxhlValue || '');
                    return;
                }

                // 2. 获取样品含量和初水分值
                const ypzxxhl = new Decimal(ypzxxhlValue);
                ...
                // 格式化结果 (依赖主组件的 formatPrecision 方法)
