                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><el-input v-model="c" class="tembum" @input="onNumberInput($event, Details, 'c')"
                                    :readonly="Details.status == '2' || isJCRY"></el-input>/<el-input v-model="ph"
                            ...
                            <td><el-input v-model="Details.ddgbh" updateDetails
                                    :readonly="Details.status == '2' || isJCRY"></el-input></td>
                                ...
                                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                                    <tbody>
                                        ...
                                        <template v-for="(item, index) in Details.infoList">
                                            <tr :key="`row1-${index}`" class="sample-split"
                                                ...
                                                <td rowspan="2"><el-input v-model="item.sampleName"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input></td>
                                                <td><el-input v-model="item.zxph" inputmode="decimal"
                                                        @input="onNumberInput($event, item, 'zxph')"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input></td>
                                                <td><el-input v-model="item.m" inputmode="decimal"
                                                        @input="onNumberInput($event, item, 'm')"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input></td>
                                                <td><el-input v-model="item.v2" inputmode="decimal"
                                                        @input="onNumberInput($event, item, 'v2')"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input></td>

                                                <td><el-input :value="computew(item)" readonly
                                            ...
                                            <tr :key="`row2-${index}`">
                                                <td><el-input v-model="item.compareZxph" inputmode="decimal"
                                                        @input="onNumberInput($event, item, 'compareZxph')"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input>
                                                </td>
                                                ...
                                                <td><el-input :value="computeCompareW(item)" readonly
                                                        @blur="updateCompareValues(item)"></el-input>
                                                </td>
                                            </tr>
                                        </template>
                                        ...
                                        <tr>
                                                    ...
                                                    <span class="math-formula">
                                                        <span class="math-fraction">
                                                            <span class="math-numerator">
                                                                (V<sub>2</sub> - V<sub>1</sub>) × c × 83.99
                                                            </span>
                                                            <span class="math-fraction-line"></span>
                                                            <span class="math-denominator">m × 1000</span>
                                                        </span>
                                                    </span>
                                                    × 100
                                            ...
                                            <td colspan="3" style="text-align:left">空白消耗盐酸标准溶液体积V<sub>1</sub><el-input
                                                    v-model="Details.v1" inputmode="decimal"
                                                    @input="onNumberInput($event, Details, 'v1')" style="width:70px"
...
export default {
    ...
    computed: {
        ...
        formulaTipsList() {
            ...
            (this.Details.infoList || []).forEach((row, idx) => {
                ...
                if (!row.v2 || !row.compareV2) rowEmpty.push('消耗体积 v₂');
        ...
        formulaTips() {
            const empty = [];
            if (!this.c) empty.push('第一行浓度');
            if (!this.Details.v1) empty.push('空白消耗盐酸标准溶液体积');
    ...
    methods: {
        ...
        computew(item) {
            // 检查必需的值是否都已输入
            const requiredValues = [this.c, this.Details.v1, item.v2, item.m];
            ...
            try {
                ...
                const one = v2.minus(v1)
                const two = one.times(c).times(new Decimal(83.99))
                const three = m.times(new Decimal(1000))
                const result = two.dividedBy(three).times(100); // *** 逻辑修正：公式中没有 *100 ***
        ...
        // 计算对比X值的方法
        computeCompareW(item) {
            // 检查必需的值是否都已输入
            const requiredValues = [this.c, this.Details.v1, item.compareV2, item.compareM];
            if (!this.hasRequiredValues(requiredValues)) {
        ...
        updateCompareValues(item) {
            this.$nextTick(() => {
                // *** 逻辑修改：确保 item.w/compareW 被赋值 ***
                item.w = this.computew(item);
                item.compareW = this.computeCompareW(item);

                const Jsg = item.w;
                const JscompareG = item.compareW;

                if (Jsg !== '' && JscompareG !== '' && Jsg !== null && JscompareG !== null) {
                    const avg = new Decimal(Jsg).plus(new Decimal(JscompareG)).dividedBy(2);
                    const absoluteDiff = Decimal.abs(new Decimal(Jsg).minus(new Decimal(JscompareG)));
                    ...
                    this.$set(item, 'absoluteDifference', absoluteDiff.toFixed(2)); // (absoluteDiff.toFixed(2) 是原始逻辑，予以保留)

                    // *** 逻辑修改：新增负数校验 ***
                    const average_val = parseFloat(item.average);
                    if ((new Decimal(Jsg).lessThan(0)) || (new Decimal(JscompareG).lessThan(0)) || (!isNaN(average_val) && average_val < 0)) {
                        const now = Date.now();
