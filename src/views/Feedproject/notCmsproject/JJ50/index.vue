                                ...
                                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                                    <tbody>
                                        <tr v-if="formulaTipsList">
                                            <td colspan="6" style="border:0;padding:0;text-align:left">
                                        ...
                                        <template v-for="(item, index) in Details.infoList">
                                            <tr :key="`row1-${index}`" class="sample-split"
                                                ...
                                                <td rowspan="2"><el-input v-model="item.sampleName"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input></td>
                                                <td><el-input v-model="item.a"
                                                        :readonly="Details.status == '2' || isJCRY"
                                                        @input="updateCompareValues(item)"
                                                        @blur="updateCompareValues(item)"></el-input>
                                                </td>
                                                <td rowspan="2"><el-input v-model="item.average"></el-input>
                                                </td>
                                                <td rowspan="2">
                                                </td>
                                                <td rowspan="2">
                                            ...
                                            <tr :key="`row2-${index}`">
                                                <td><el-input v-model="item.compareA"
                                                        :readonly="Details.status == '2' || isJCRY"
                                                        @input="updateCompareValues(item)"
                                                        @blur="updateCompareValues(item)"></el-input>
                                                </td>
...
export default {
    ...
    computed: {
        ...
        formulaTipsList() {
            ...
            (this.Details.infoList || []).forEach((row, idx) => {
                const rowEmpty = [];
                if (!row.a || !row.compareA) rowEmpty.push('容重');
    ...
    methods: {
        ...
        validateEmpty() {
            ...
            /* ---- 逐行样品字段 ---- */
            (this.Details.infoList || []).forEach((row, idx) => {
                if (!row.a) rowEmpty.push('容重');
                if (!row.compareA) empty.push(`第${idx + 1}行对比容重`);
            });
        ...
        updateCompareValues(item) {
            this.$nextTick(() => {
                // 检查 a 和 compareA 是否都有有效值
                const aVal = item.a;
                const compareAVal = item.compareA;

                if (aVal && compareAVal && !isNaN(aVal) && !isNaN(compareAVal)) {
                    const a = new Decimal(aVal);
                    const compareA = new Decimal(compareAVal);
                    const avg = a.plus(compareA).dividedBy(2);

                    // toFixed(1) 保留一位小数，自动补0（如：10 → "10.0"）
                    this.$set(item, 'average', avg.toFixed(1));
                } else {
