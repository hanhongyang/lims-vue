                        ...
                        <tr>
                            <td colspan="9" style="padding: 0; border: 0;">
                                <table cellpadding="0" cellspacing="0" width="100%" style="table-layout: fixed;">
                                    <tr>
                                        <td style="width: 19.05%;">ZEN标准液浓度(ug/kg)</td>
                                        <td style="width: 13.33%;">0</td>
                                        <td style="width: 13.33%;">20</td>
                                        <td style="width: 13.33%;">70</td>
                                        <td style="width: 13.33%;">350</td>
                                        <td style="width: 13.33%;">1500</td>
                                ...
                                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>待测液中ZEN含量 C(ng/mL)(w)</td>
                                            <td>样品中ZEN含量(ug/kg)</td>
                                            <td>鲜样中ZEN含量(ug/kg)</td>
                                            <td>操作</td>
                                        </tr>
                                        <template v-for="(item, index) in Details.infoList">
                                            <tr :key="`row1-${index}`" class="sample-split"
                                                ...
                                                <td><el-input v-model="item.c" @input="bringCtoSample(item)"
                                                        :readonly="Details.status == '2' || isJCRY"></el-input></td>
                                                <td><el-input v-model="item.x1"></el-input></td>
                                                <!-- <td><el-input :value="item.ypzxxhl" placeholder="x1" readonly
                                                        @blur="updateCompareValues(item)"></el-input></td> -->
                                                <td><el-input v-model="item.xyzhl" inputmode="decimal"></el-input></td>
                                                <td>
...
export default {
    ...
    computed: {
        ...
        formulaTipsList() {
            ...
            (this.Details.infoList || []).forEach((row, idx) => {
                const rowEmpty = [];
                if (!row.c) rowEmpty.push('待测液中ZEN含量');
    ...
    methods: {
        ...
        validateEmpty() {
            ...
            /* ---- 逐行样品字段 ---- */
            (this.Details.infoList || []).forEach((row, idx) => {
                if (!row.c) empty.push(`第${idx + 1}行待测液中ZEN含量`);
        ...
        computex(item) {
            ...
            try {
                ...
                const numerator1 = c.dividedBy(m);
                const result = numerator1.times(5);

                const formattedResult = this.formatPrecision(result);
                this.$set(item, 'ypzxxhl', formattedResult); // *** 逻辑修改：确保保存 ***
        ...
        async submitForm(preventNavigation = false) {
            ...
            // *** 逻辑修改：Bug修复，确保 xxryph 和 xgd 都被提交 ***
            submitData.xxryph = this.Details.xxryph; // 确保"甲醇"字段被提交
            submitData.xgd = this.Details.xgd;       // 确保"吸光度"字段被提交
        ...
        calculateSingleFreshSample(item, csfValue) {
            // *** 逻辑修改：JJ18/JJ19/JJ51/JJ53的鲜样计算依赖 ypzxxhl (样品中含量)，而非 average ***
            try {
                // 检查 ypzxxhl 是否有效
