          ...
          <tbody>
            ...
            <tr>
              ...
              <td rowspan="2"><el-input v-model="Details.temperature" class="tembum"
                  :readonly="Details.status == '2' || isJCRY" :id="'cell-0-0'"
                  @keydown.native="e => handleKeyNav(e, 0, 0)"></el-input>/<el-input v-model="Details.humidity"
                  class="tembum" :readonly="Details.status == '2' || isJCRY" :id="'cell-1-0'"
                  @keydown.native="e => handleKeyNav(e, 1, 0)"></el-input></td>
              <td rowspan="2"><el-input v-model="oneBeforeTime" class="HgTime" updateDetails
                  :readonly="Details.status == '2' || isJCRY" :id="'cell-0-1'"
                  @keydown.native="e => handleKeyNav(e, 0, 1)"></el-input>点~<el-input v-model="oneAfterTime"
                  class="HgTime" updateDetails :readonly="Details.status == '2' || isJCRY" :id="'cell-1-1'"
                  @keydown.native="e => handleKeyNav(e, 1, 1)"></el-input>点</td>
              <td rowspan="2"><el-input v-model="twoBeforeTime" class="HgTime" updateDetails
                  :readonly="Details.status == '2' || isJCRY" :id="'cell-0-2'"
                  @keydown.native="e => handleKeyNav(e, 0, 2)"></el-input>点~<el-input v-model="twoAfterTime"
                  class="HgTime" updateDetails :readonly="Details.status == '2' || isJCRY" :id="'cell-1-2'"
                  @keydown.native="e => handleKeyNav(e, 1, 2)"></el-input>点</td>
              <td><el-input v-model="instrumentNameOne" updateDetails :readonly="Details.status == '2' || isJCRY"
                  :id="'cell-0-3'" @keydown.native="e => handleKeyNav(e, 0, 3)"></el-input>
              </td>
              <td><el-input v-model="instrumentNoOne" :readonly="Details.status == '2' || isJCRY" :id="'cell-0-4'"
                  @keydown.native="e => handleKeyNav(e, 0, 4)"></el-input></td>
              <td rowspan="2"><el-input v-model="Details.testBasis" :readonly="Details.status == '2' || isJCRY"
                  :id="'cell-0-5'" @keydown.native="e => handleKeyNav(e, 0, 5)"></el-input>
              </td>
              <td><el-input v-model="testLocationOne" :readonly="Details.status == '2' || isJCRY" :id="'cell-0-6'"
                  @keydown.native="e => handleKeyNav(e, 0, 6)"></el-input></td>
            </tr>
            <tr>
              <td><el-input v-model="instrumentNameTwo" updateDetails :readonly="Details.status == '2' || isJCRY"
                  :id="'cell-1-3'" @keydown.native="e => handleKeyNav(e, 1, 3)"></el-input>
              </td>
              <td><el-input v-model="instrumentNoTwo" :readonly="Details.status == '2' || isJCRY" :id="'cell-1-4'"
                  @keydown.native="e => handleKeyNav(e, 1, 4)"></el-input></td>
              <td><el-input v-model="testLocationTwo" :readonly="Details.status == '2' || isJCRY" :id="'cell-1-6'"
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                  <tbody>
                    ...
                    <template v-for="(item, index) in Details.infoList">
                      <tr :key="`row1-${index}`" class="sample-split" :class="{ 'first-sample': index === 0 }">
                        ...
                        <td rowspan="2"><el-input v-model="item.sampleName"
                            :readonly="Details.status == '2' || isJCRY"></el-input></td>
                        <td><el-input v-model="item.clmh" :id="'cell-' + (index * 2 + 2) + '-0'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 0)"></el-input
                            :readonly="Details.status == '2' || isJCRY"></td>
                        <td><el-input v-model="item.m2" inputmode="decimal" @input="onNumberInput($event, item, 'm2')"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 1)"></el-input>
                        </td>
                        <td><el-input v-model="item.m1" inputmode="decimal" @input="onNumberInput($event, item, 'm1')"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 2)"></el-input>
                        </td>
                        <td><el-input v-model="item.m3" inputmode="decimal" @input="onNumberInput($event, item, 'm3')"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 3)"></el-input>
                        </td>
                        <td><el-input v-model="item.m4" inputmode="decimal" @input="onNumberInput($event, item, 'm4')"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 4)"></el-input>
                        </td>
                        ...
                        <td><el-input v-model="item.upperM1" inputmode="decimal"
                            ...
                            :id="'cell-' + (index * 2 + 2) + '-5'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 5)"></el-input></td>
                      ...
                      <tr :key="`row2-${index}`">
                        <td><el-input v-model="item.compareClmh" :readonly="Details.status == '2' || isJCRY"
                            :id="'cell-' + (index * 2 + 3) + '-0'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 0)"></el-input></td>
                        <td><el-input v-model="item.compareM2" inputmode="decimal"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 1)"></el-input></td>
                        <td><el-input v-model="item.compareM1" inputmode="decimal"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 2)"></el-input></td>
                        <td><el-input v-model="item.compareM3" inputmode="decimal"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 3)"></el-input></td>
                        <td><el-input v-model="item.compareM4" inputmode="decimal"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 4)"></el-input></td>
                        ...
                        <td><el-input v-model="item.compareUpperM1" inputmode="decimal"
                            ...
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 5)"></el-input>
                        </td>
                    ...
                    <tr>
                      ...
                      <td colspan="11" style="text-align:left">两个平行样测定绝对值相差不得超过0.2%，否则重做。若此次平行样绝对差值仍然超过0.2%，但四次测试结果未超过
                        0.5%，取四次测定的算术平均值作为结果。</td>
                    </tr>
                    <tr>
                      <td colspan="2">备注</td>
                      <td colspan="11" style="text-align:left">直至两次连续称量值差小于试样质量的0.005g(取样量为5g时)。</td>
                    </tr>
                  </tbody>
                </table>
              </td>
...
export default {
  name: "Moisturewater", // *** 逻辑修正：Name 应为 Moisturewater (JJ49) ***
  mixins: [csfhyMixinForFeedwater], // *** 逻辑修改：使用 mixin ***
  components: {
  ...
  methods: {
    ...
    // *** 逻辑修改：新增 mixin 存根 ***
    csfhy() {
      // this.csfhyMixinHandler(); // (JJ49/JJ48/JJ01没有初水分按钮)
    },
    ...
    computex(item) {
      ...
      try {
        ...
        const formattedResult = this.formatPrecision(result, 2);
    ...
    // 精度格式化方法 - 添加 decimalPlaces 参数
    formatPrecision(value, decimalPlaces = null) {
      ...
      // 如果传入了指定的小数位数，直接使用 toFixed 补0
      if (decimalPlaces !== null) {
        return num.toFixed(decimalPlaces);
      }

      // 优先使用配置的小数位，其次使用有效数字位
      if (this.Details.decimalPlaces) {
        const dp = parseInt(this.Details.decimalPlaces);
    ...
    updateCompareValues(item) {
      ...
      this.$nextTick(() => {
        ...
        if (val1 !== '' && val2 !== '' && val1 !== null && val2 !== null) {
          ...
          this.$set(item, 'average', this.formatPrecision(avg, 1));
    ...
    async submitForm(preventNavigation = false) {
      ...
      // (JJ49/JJ48/JJ01 是水分单, 不依赖 mixin 计算鲜样含量)
    ...
    // (JJ49 原始文件误用了 submitForm, 此处修正为 submitCheck)
    async submitCheck() {
