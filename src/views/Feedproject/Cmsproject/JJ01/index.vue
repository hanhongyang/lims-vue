          ...
          <tbody>
            ...
            <tr>
              <td rowspan="2"><el-date-picker v-model="Details.testDate" type="date" class="custom-padding"
                  :editable="false" :readonly="isJCRY || Details.status == '2'">
                </el-date-picker></td>
              <td rowspan="2"><el-input v-model="Details.temperature" class="tembum"
                  :readonly="isJCRY || Details.status == '2'" :id="'cell-0-0'"
                  @keydown.native="e => handleKeyNav(e, 0, 0)"></el-input>/<el-input v-model="Details.humidity"
                  class="tembum" :readonly="isJCRY || Details.status == '2'" :id="'cell-1-0'"
                  @keydown.native="e => handleKeyNav(e, 1, 0)"></el-input></td>
              <td rowspan="2"><el-input v-model="oneBeforeTime" class="HgTime" updateDetails
                  :readonly="isJCRY || Details.status == '2'" :id="'cell-0-1'"
                  @keydown.native="e => handleKeyNav(e, 0, 1)"></el-input>点~<el-input v-model="oneAfterTime"
                  class="HgTime" updateDetails :readonly="isJCRY || Details.status == '2'" :id="'cell-1-1'"
                  @keydown.native="e => handleKeyNav(e, 1, 1)"></el-input>点</td>
              <td rowspan="2"><el-input v-model="twoBeforeTime" class="HgTime" updateDetails
                  :readonly="isJCRY || Details.status == '2'" :id="'cell-0-2'"
                  @keydown.native="e => handleKeyNav(e, 0, 2)"></el-input>点~<el-input v-model="twoAfterTime"
                  class="HgTime" updateDetails :readonly="isJCRY || Details.status == '2'" :id="'cell-1-2'"
                  @keydown.native="e => handleKeyNav(e, 1, 2)"></el-input>点</td>
              <td><el-input v-model="instrumentNameOne" updateDetails :readonly="isJCRY || Details.status == '2'"
                  :id="'cell-0-3'" @keydown.native="e => handleKeyNav(e, 0, 3)"></el-input>
              </td>
              <td><el-input v-model="instrumentNoOne" :readonly="isJCRY || Details.status == '2'" :id="'cell-0-4'"
                  @keydown.native="e => handleKeyNav(e, 0, 4)"></el-input></td>
              <td rowspan="2"><el-input v-model="Details.testBasis" :readonly="isJCRY || Details.status == '2'"
                  :id="'cell-0-5'" @keydown.native="e => handleKeyNav(e, 0, 5)"></el-input>
              </td>
              <td><el-input v-model="testLocationOne" :readonly="isJCRY || Details.status == '2'" :id="'cell-0-6'"
                  @keydown.native="e => handleKeyNav(e, 0, 6)"></el-input></td>
            </tr>
            <tr>
              <td><el-input v-model="instrumentNameTwo" updateDetails :readonly="isJCRY || Details.status == '2'"
                  :id="'cell-1-3'" @keydown.native="e => handleKeyNav(e, 1, 3)"></el-input>
              </td>
              <td><el-input v-model="instrumentNoTwo" :readonly="isJCRY || Details.status == '2'" :id="'cell-1-4'"
                  @keydown.native="e => handleKeyNav(e, 1, 4)"></el-input></td>
              <td><el-input v-model="testLocationTwo" :readonly="isJCRY || Details.status == '2'" :id="'cell-1-6'"
                  @keydown.native="e => handleKeyNav(e, 1, 6)"></el-input></td>
            </tr>
            <tr>
                  ...
                  <tbody>
                    ...
                    <template v-for="(item, index) in Details.infoList">
                      <tr :key="`row1-${index}`" class="sample-split" :class="{ 'first-sample': index === 0 }">
                        <td rowspan="2">
                          <!-- 样品编号不参与跳转 -->
                          <el-input v-model="item.sampleNo" :readonly="Details.status == '2' || isJCRY"></el-input>
                        </td>
                        <td rowspan="2">
                          <el-input v-model="item.sampleName" :readonly="Details.status == '2' || isJCRY"></el-input>
                        </td>

                        <!-- 【列 0】 称量皿号 -->
                        <td>
                          <el-input v-model="item.clmh" :readonly="Details.status == '2' || isJCRY"
                            :id="'cell-' + (index * 2 + 2) + '-0'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 0)">
                          </el-input>
                        </td>
                        <!-- 【列 1】 m2 -->
                        <td>
                          <el-input v-model="item.m2" inputmode="decimal" @input="onNumberInput($event, item, 'm2')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 2) + '-1'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 1)">
                          </el-input>
                        </td>
                        <!-- 【列 2】 m1 -->
                        <td>
                          <el-input v-model="item.m1" inputmode="decimal" @input="onNumberInput($event, item, 'm1')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 2) + '-2'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 2)">
                          </el-input>
                        </td>
                        <!-- 【列 3】 m3 -->
                        <td>
                          <el-input v-model="item.m3" inputmode="decimal" @input="onNumberInput($event, item, 'm3')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 2) + '-3'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 3)">
                          </el-input>
                        </td>
                        <!-- 【列 4】 m4 -->
                        <td>
                          <el-input v-model="item.m4" inputmode="decimal" @input="onNumberInput($event, item, 'm4')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 2) + '-4'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 4)">
                          </el-input>
                        </td>
                        ...
                        <!-- 【列 5】 初水分 -->
                        <td>
                          <el-input v-model="item.upperM1" inputmode="decimal"
                            @input="onNumberInput($event, item, 'upperM1')" :readonly="Details.status == '2' || isJCRY"
                            :id="'cell-' + (index * 2 + 2) + '-5'"
                            @keydown.native="handleKeyNav($event, index * 2 + 2, 5)">
                          </el-input>
                        </td>
                        ...
                        <td rowspan="2">
                          ...
                          <template v-if="isShow">
                            ...
                            <span v-if="item.isReset == 1 || item.isReset == '1'"
                              style="color: #E6A23C; font-size: 12px;">已申请复检</span>
                          </template>
                        </td>
                      </tr>
                      ...
                      <tr :key="`row2-${index}`">
                        <!-- 【列 0】 对比称量皿 -->
                        <td>
                          <el-input v-model="item.compareClmh" :readonly="Details.status == '2' || isJCRY"
                            :id="'cell-' + (index * 2 + 3) + '-0'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 0)">
                          </el-input>
                        </td>
                        <!-- 【列 1】 对比 m2 -->
                        <td>
                          <el-input v-model="item.compareM2" inputmode="decimal"
                            @input="onNumberInput($event, item, 'compareM2')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 3) + '-1'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 1)">
                          </el-input>
                        </td>
                        <!-- 【列 2】 对比 m1 -->
                        <td>
                          <el-input v-model="item.compareM1" inputmode="decimal"
                            @input="onNumberInput($event, item, 'compareM1')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 3) + '-2'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 2)">
                          </el-input>
                        </td>
                        <!-- 【列 3】 对比 m3 -->
                        <td>
                          <el-input v-model="item.compareM3" inputmode="decimal"
                            @input="onNumberInput($event, item, 'compareM3')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 3) + '-3'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 3)">
                          </el-input>
                        </td>
                        <!-- 【列 4】 对比 m4 -->
                        <td>
                          <el-input v-model="item.compareM4" inputmode="decimal"
                            @input="onNumberInput($event, item, 'compareM4')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 3) + '-4'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 4)">
                          </el-input>
                        </td>
                        ...
                        <!-- 【列 5】 对比初水分 -->
                        <td>
                          <el-input v-model="item.compareUpperM1" inputmode="decimal"
                            @input="onNumberInput($event, item, 'compareUpperM1')"
                            :readonly="Details.status == '2' || isJCRY" :id="'cell-' + (index * 2 + 3) + '-5'"
                            @keydown.native="handleKeyNav($event, index * 2 + 3, 5)">
                          </el-input>
                        </td>
                    ...
                    <tr>
                      <td colspan="2">备注</td>
                      <td colspan="11" style="text-align:left">如果 m₃ 与 m₄ 的变化小于试样质量的 0.1%，以 m₃ 计算水分；否则需第三次干燥。</td>
                    </tr>
                  </tbody>
                </table>
              </td>
...
import { feedwatertask, feedwatertaskUp, listTask, saveCheck, XZPic, csfhy, retest, testSubmit, usersign, backToSubmit } from "@/api/hyd";
...
export default {
  ...
  mounted() {
    ...
    // (JJ01 没有 loadSignatureImages, 已在 SjtasInfo 逻辑中补充 XZPic)
  },
  ...
  methods: {
    handleKeyNav(e, currentRow, currentCol) {
      // 允许按键：Enter, Up, Down, Left, Right
      ...
      if (e.key === 'Enter' || e.key === 'ArrowDown') {
        nextRow++;
        shouldMove = true;
        // 只有 Enter 键才需要阻止默认行为(避免表单提交等)，箭头键通常保留原生滚动或让光标不动即可
      ...
      // 左箭头
        ...
        // 只有光标在最左边（index 0）且没有选中文本时，才跳转到左边的格子
        if (targetInput.selectionStart === 0 && targetInput.selectionEnd === 0) {
          nextCol--;
          shouldMove = true;
          e.preventDefault(); // 阻止光标继续向左撞墙
        }
      }
      // 右箭头
      else if (e.key === 'ArrowRight') {
        // 只有光标在最右边（文本长度）且没有选中文本时，才跳转到右边的格子
        if (targetInput.selectionStart === targetInput.value.length && targetInput.selectionEnd === targetInput.value.length) {
          nextCol++;
          shouldMove = true;
          e.preventDefault();
        }
      ...
      if (shouldMove) {
        ...
        // ElementUI 的 el-input 会把 id 透传给内部的 input 标签，或者我们可以查找 wrapper 下的 input
        let nextEl = document.getElementById(nextId);

        // 如果 nextEl 存在（说明是 input 元素），直接 focus
        // 注意：ElementUI 某些版本 id 是在 div 上，某些在 input 上。
        // 为了兼容，如果获取到的是 div (className 包含 el-input)，则找内部 input
        if (nextEl) {
          if (nextEl.tagName !== 'INPUT') {
            nextEl = nextEl.querySelector('input');
          }
          if (nextEl) {
            nextEl.focus();
            // 选中所有文本（可选，方便直接修改）
    ...
    csfhy() {
      // this.csfhyMixinHandler(); // (JJ01 原始文件未调用)
    },
    ...
    computex1(item) {
      ...
      try {
        ...
        const formattedResult = result.toFixed(2);
        console.log(`计算X1: upperM1=${item.upperM1}, x=${xValue}, 结果=${formattedResult}`);
        ...
        if (val1 !== '' && val2 !== '' && val1 !== null && val2 !== null) {
          try {
            ...
            if (avg.greaterThan(0)) {
              console.log('平均值:', avg);
              // 规则反转：≤15 % 用相对偏差，＞15 % 用绝对差值
              if (avg.greaterThanOrEqualTo(15)) {
                // |平行样1 – 平均值| / 平均值 × 100
                const relDiff = new Decimal(val1).minus(avg).abs().dividedBy(avg).times(100);
                console.log('相对偏差:', relDiff + '%');
                diffDisplay = relDiff.toFixed(2) + '%';
              } else {
                // 绝对差值 |平行样1 – 平行样2|
                console.log('绝对差值:', absoluteDiff);
            ...
            this.$set(item, 'average', avg.toDecimalPlaces(1).toFixed(1));
    ...
    // *** 逻辑修改：新增 loadSignatureImages (使用 XZPic) ***
    async loadSignatureImages() {
      try {
        if (this.Details.testUserSignature) {
          const res = await XZPic(this.Details.testUserSignature);
          this.signTypes.检测人 = URL.createObjectURL(res);
          this.testUserSignature = this.Details.testUserSignature;
        }
        if (this.Details.checkUserSignature) {
          const res1 = await XZPic(this.Details.checkUserSignature);
          this.signTypes.核对人 = URL.createObjectURL(res1);
          this.checkUserSignature = this.Details.checkUserSignature;
        }
      } catch (error) {
    ...
    async submitTest() {
      ...
      const empty = this.validateEmpty();
      if (empty.length) {
        this.$message.error(`请完善以下字段再提交：${empty.join('、')}`);
        return;
      }
