        ...
        <table cellpadding="0" cellspacing="0" width="100%" id="exportTable" style="text-align: center;">
          <tbody>
            ...
            <tr>
              ...
              <td rowspan="2"><el-input v-model="Details.symph" class="tembum"
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>滤袋号</td>
                      <td>试样质量 m（g）</td>
                      <td>提取前烘干后样品+滤袋+硅藻土质量m<sub>1</sub>（g）</td>
                      <td>提取后烘干后样品+滤袋+硅藻土质量m<sub>2</sub>（g）</td>
                      <td>试样中粗脂肪含量 EE%</td>
                      <td>平均值%</td>
                      <td>绝对差值/相对相差%</td>
                      <td>鲜样中粗脂肪含量 EE%</td>
                    ...
                    <template v-for="(item, index) in Details.infoList">
                      <tr :key="`row1-${index}`" class="sample-split" :class="{ 'first-sample': index === 0 }">
                        ...
                        <td rowspan="2"><el-input v-model="item.xyzhl" readonly></el-input>
                        </td>
                        <td rowspan="2">
    ...
    <div class="fixed-bottom">
      <div style="margin-top: 20px; text-align: center;">
        <el-button type="primary" icon="el-icon-check" @click="saveForm"
          v-if="isRec || (Details.status == '1' || Details.status == '6')">保存</el-button>
        <el-button type="success" icon="el-icon-upload2" @click="submitTest"
          v-if="isRec || (Details.status == '1' || Details.status == '6')">提交</el-button>
...
export default {
  ...
  computed: {
    ...
    formulaTipsList() {
      ...
      (this.Details.infoList || []).forEach((row, idx) => {
        ...
        if (!row.m1 || !row.compareM1) rowEmpty.push('提取前烘干后样品+滤袋+硅藻土质量 m₁');
        if (!row.m2 || !row.compareM2) rowEmpty.push('提取后烘干后样品+滤袋+硅藻土质量 m₂');
        if (rowEmpty.length) {
  ...
  mounted() {
    ...
    if (this.$route.params.taskInfo) {
      ...
      if (this.$route.params.SjtaskInfo.testUserSignature) {
        XZPic(this.$route.params.SjtaskInfo.testUserSignature).then((res) => {
          ...
          this.testUserSignature = this.Details.testUserSignature; // *** 加载签名ID ***
        })
      }
      if (this.$route.params.SjtaskInfo.checkUserSignature) {
        XZPic(this.$route.params.SjtaskInfo.checkUserSignature).then((res1) => {
          ...
          this.checkUserSignature = this.Details.checkUserSignature; // *** 加载签名ID ***
        })
      }
    ...
    this.isPadPc()
    this.initTimes(); // *** 原始逻辑是 initTimes, 非 initData ***
    // 在初始化后查询初水分数据并计算鲜样含量
    this.$nextTick(() => {
      // *** 确保 infoList 存在 ***
  ...
  methods: {
    ...
    validateEmpty() {
      ...
      /* ---- 逐行样品字段 ---- */
      (this.Details.infoList || []).forEach((row, idx) => {
        if (!row.m1) empty.push(`第${idx + 1}行提取前烘干后样品+滤袋+硅藻土质量 m₁`);
        if (!row.m) empty.push(`第${idx + 1}行试样质量 m`);
        if (!row.m2) empty.push(`第${idx + 1}行提取后烘干后样品+滤袋+硅藻土质量 m₂`);
        if (!row.compareM1) empty.push(`第${idx + 1}行对比提取前烘干后样品+滤袋+硅藻土质量 m₁`);
        if (!row.compareM) empty.push(`第${idx + 1}行对比试样质量 m`);
        if (!row.compareM2) empty.push(`第${idx + 1}行对比提取后烘干后样品+滤袋+硅藻土质量 m₂`);
      });
    ...
    initTimes() {
      // (JJ04 的 initTimes 中没有 xxFirstTime/xxSecondTime)
    ...
    updateDetails() {
      // (JJ04 的 updateDetails 中没有 xxFirstTime/xxSecondTime)
    ...
    computex(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [item.m1, item.m2, item.m];
      if (!this.hasRequiredValues(requiredValues)) {
        this.$set(item, 'ee', ''); // *** 确保清空 ***
        return '';
      }
      ...
      try {
        ...
        // 检查分母是否为0
        if (m.equals(0)) {
          this.$set(item, 'ee', '0'); // *** 确保赋值 ***
          return '0';
        }

        const numerator = (m1.minus(m2));
        const result = numerator.dividedBy(m).times(100);

        const formattedResult = result.toFixed(2);
        this.$set(item, 'ee', formattedResult); // *** 确保保存 ***
    ...
    computeCompareX(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [item.compareM1, item.compareM2, item.compareM];
      if (!this.hasRequiredValues(requiredValues)) {
        this.$set(item, 'compareEe', ''); // *** 确保清空 ***
        return '';
      }
      ...
      try {
        ...
        // 检查分母是否为0
        if (m.equals(0)) {
          this.$set(item, 'compareEe', '0'); // *** 确保赋值 ***
          return '0';
        }
        ...
        const formattedResult = result.toFixed(2);
        this.$set(item, 'compareEe', formattedResult); // *** 确保保存 ***
        return formattedResult;
      } catch (error) {
        console.error('计算X值错误:', error);
        this.$set(item, 'compareEe', '');
        return '';
      }
    },
    //精度格式化方法
    ...
    updateCompareValues(item) {
      item.compareEe = this.computeCompareX(item);
      item.ee = this.computex(item);


      this.$nextTick(() => {
        const ee = item.ee; // *** 使用已存值 ***
        const compareEe = item.compareEe;

        if (ee !== '' && compareEe !== '' && ee !== null && compareEe !== null) {
          // ***  "绝对差值/相对相差 (%)" ***

          // 1. 计算平均值 (Decimal object)
          const avg = new Decimal(ee).plus(new Decimal(compareEe)).dividedBy(2);
          // 2. 计算绝对差值 (Decimal object)
          const absoluteDiff = Decimal.abs(new Decimal(ee).minus(new Decimal(compareEe)));

          // 3. 存入平均值 (格式化)
          // this.$set(item, 'average', this.formatPrecision(avg));
          this.$set(item, 'average', avg.toDecimalPlaces(1).toFixed(1));

          // 4. 获取格式化后的平均值 (浮点数) 用于逻辑判断
          const avgValue = parseFloat(item.average);

          if (isNaN(avgValue)) {
            this.$set(item, 'absoluteDifference', ''); // 平均值无效，清空
          } else if (avgValue < 5) {
            // 规则1：粗脂肪<5%时，使用 绝对差值 (并格式化)
            this.$set(item, 'absoluteDifference', absoluteDiff.toFixed(2));
          } else {
            // 规则2 & 3：粗脂肪>=5%时，使用 相对相差
            // (使用未格式化的 avg (Decimal object) 计算以保证精度)
            const relativeDiff = avg.equals(0) ? new Decimal(0) : absoluteDiff.dividedBy(avg).times(100);

            // 相对相差通常保留2位小数
            this.$set(item, 'absoluteDifference', relativeDiff.toFixed(2));
          }

          // *** 新增负数校验 ***
          const average_val = parseFloat(item.average);
          if ((new Decimal(ee).lessThan(0)) || (new Decimal(compareEe).lessThan(0)) || (!isNaN(average_val) && average_val < 0)) {
            const now = Date.now();
          ...
          //【修复：新增 Mixin 调用】在平均值计算完成后，调用 Mixin 计算鲜样含量
    ...
    async submitForm(preventNavigation = false) {
      ...
      // *** 调用 updateDetails ***
      this.updateDetails(); // 确保 instrumentName 等字段被组合
    ...
    async submitCheck() {
      ...
      try {
        ...
        const res = await saveCheck(submitData); // *** 调用 saveCheck ***
        if (res.code === 200) {
