          ...
          <tbody>
            ...
            <tr>
              ...
              <!-- <td rowspan="2"><el-input v-model="oneBeforeTime" class="HgTime" updateDetails
                  :readonly="isJCRY"></el-input>点~<el-input v-model="oneAfterTime" class="HgTime" updateDetails
                  :readonly="isJCRY"></el-input>点</td>
              <td rowspan="2"><el-input v-model="twoBeforeTime" class="HgTime" updateDetails
                  :readonly="isJCRY"></el-input>点~<el-input v-model="twoAfterTime" class="HgTime" updateDetails
                  :readonly="isJCRY"></el-input>点</td> -->
              ...
              <td rowspan="2" colspan="2"><el-input v-model="Details.testBasis"
                  :readonly="Details.status == '2' || isJCRY"></el-input>
              </td>
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>样品名称</td>
                      <td>坩埚号</td>
                      <td>空坩埚质量m<sub>0</sub>(g)</td>
                      <td>试样质量m(g)</td>
                      <td>灰化后灰分+坩埚质量m<sub>1</sub>(g)</td>
                      <td>粗灰分含量w (%)</td>
                      <td>平均值(%)</td>
                      <td>绝对差值(%)</td>
                      <td>鲜样中灰分含量(%)</td>
                    ...
                    <template v-for="(item, index) in Details.infoList">
                      <tr :key="`row1-${index}`" class="sample-split" :class="{ 'first-sample': index === 0 }">
                        ...
                        <td rowspan="2"><el-input v-model="item.sampleName"
                            :readonly="Details.status == '2' || isJCRY"></el-input></td>
                        <td><el-input v-model="item.compareGgh" :readonly="Details.status == '2' || isJCRY"></el-input>
                        </td>
                        <td><el-input v-model="item.m0" inputmode="decimal" @input="onNumberInput($event, item, 'm0')"
                            :readonly="Details.status == '2' || isJCRY"></el-input>
                        </td>
                        ...
                        <td><el-input :value="computex(item)" readonly inputmode="decimal"
                      ...
                      <tr :key="`row2-${index}`">
                        ...
                        <td><el-input v-model="item.compareM0" inputmode="decimal"
                            @input="onNumberInput($event, item, 'compareM0')"
                            :readonly="Details.status == '2' || isJCRY"></el-input></td>
                        ...
                        <td><el-input v-model="item.compareM1" inputmode="decimal"
                            ...
                            :readonly="Details.status == '2' || isJCRY"></el-input></td>
                        <td><el-input :value="computeCompareX(item)" readonly inputmode="decimal"
                            @blur="updateCompareValues(item)"></el-input></td>
                      </tr>
                    </template>
            ...
            <tr>
              ...
              <td>允许误差</td>
              <td colspan="2" style="text-align:left">粗灰分含量在 5%以上时，允许绝对差值≤0.21%;<br />粗灰分含量在 5%以下时，允许绝对差值≤0.09%</td>
              <td colspan="2" style="text-align:left">箱式电炉第一次<el-input v-model="oneBeforeTime" class="HgTime"
                  updateDetails :readonly="Details.status == '2' || isJCRY"></el-input>点~<el-input
                  v-model="oneAfterTime" class="HgTime" updateDetails
                  :readonly="Details.status == '2' || isJCRY"></el-input>点<br />箱式电炉第二次<el-input v-model="twoBeforeTime"
                  class="HgTime" updateDetails :readonly="Details.status == '2' || isJCRY"></el-input>点~<el-input
                  v-model="twoAfterTime" class="HgTime" updateDetails
...
export default {
  ...
  computed: {
    ...
    formulaTipsList() {
      ...
      (this.Details.infoList || []).forEach((row, idx) => {
        const rowEmpty = [];
        if (!row.m0 || !row.compareM0) rowEmpty.push('空坩埚质量m₀');
        if (!row.m || !row.compareM) rowEmpty.push('试样质量 m');
        if (!row.m1 || !row.compareM1) rowEmpty.push('灰化后灰分+坩埚质量 m₁');
  ...
  methods: {
    ...
    validateEmpty() {
      ...
      /* ---- 逐行样品字段 ---- */
      (this.Details.infoList || []).forEach((row, idx) => {
        if (!row.m1) empty.push(`第${idx + 1}行灰化后灰分+坩埚质量 m₁`);
        if (!row.m) empty.push(`第${idx + 1}行试样质量 m`);
        if (!row.m0) empty.push(`第${idx + 1}行空坩埚质量m₀`);
        if (!row.compareM1) empty.push(`第${idx + 1}行对比灰化后灰分+坩埚质量 m₁`);
        if (!row.compareM) empty.push(`第${idx + 1}行对比试样质量 m`);
        if (!row.compareM0) empty.push(`第${idx + 1}行对比空坩埚质量m₀`);
      });
    ...
    computex(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [item.m1, item.m0, item.m];
      if (!this.hasRequiredValues(requiredValues)) {
        this.$set(item, 'w', ''); // *** 逻辑修改：确保清空 ***
        return '';
      }

      try {
        const m1 = new Decimal(item.m1);
        const m0 = new Decimal(item.m0);
        const m = new Decimal(item.m);

        // 检查分母是否为0
        if (m.equals(0)) {
          this.$set(item, 'w', '0'); // *** 逻辑修改：确保赋值 ***
          return '0';
        }


        const numerator = m1.minus(m0);
        const numerator1 = numerator.dividedBy(m);
        const result = numerator1.times(100);

        // const formattedResult = this.formatPrecision(result);
        const formattedResult = new Decimal(result).toFixed(2);
        this.$set(item, 'w', formattedResult); // *** 逻辑修改：确保保存 ***
    ...
    computeCompareX(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [item.compareM1, item.compareM0, item.compareM];
      if (!this.hasRequiredValues(requiredValues)) {
        this.$set(item, 'compareW', ''); // *** 逻辑修改：确保清空 ***
        return '';
      }

      try {
        const m1 = new Decimal(item.compareM1);
        const m0 = new Decimal(item.compareM0);
        const m = new Decimal(item.compareM);

        // 检查分母是否为0
        if (m.equals(0)) {
          this.$set(item, 'compareW', '0'); // *** 逻辑修改：确保赋值 ***
          return '0';
        }
        ...
        // const formattedResult = this.formatPrecision(result);
        const formattedResult = new Decimal(result).toFixed(2);
        this.$set(item, 'compareW', formattedResult); // *** 逻辑修改：确保保存 ***
        return formattedResult;
      } catch (error) {
        console.error('计算X值错误:', error);
        this.$set(item, 'compareW', '');
        return '';
      }
    },
    //精度格式化方法
    ...
    updateCompareValues(item) {
      item.compareW = this.computeCompareX(item);
      item.w = this.computex(item);

      this.$nextTick(() => {
        const w = item.w;
        const compareW = item.compareW;

        if (w !== '' && compareW !== '' && w !== null && compareW !== null) {
          const avg = new Decimal(w).plus(new Decimal(compareW)).dividedBy(2);
          const absoluteDiff = Decimal.abs(new Decimal(w).minus(new Decimal(compareW)));
          // *** 逻辑修改：使用 avg 计算相对相差 ***
          const relativeDiff = avg.equals(0) ? new Decimal(0) : absoluteDiff.dividedBy(avg).times(100);
          const absoluteDiffOnly = absoluteDiff;
          // this.$set(item, 'average', this.formatPrecision(avg)); // *** 逻辑修改：使用精度格式化 ***
          this.$set(item, 'average', avg.toDecimalPlaces(1).toFixed(1));
          this.$set(item, 'absoluteDifference', absoluteDiffOnly.toFixed(2));

          // *** 逻辑修改：新增负数校验 ***
          const average_val = parseFloat(item.average);
          if ((new Decimal(w).lessThan(0)) || (new Decimal(compareW).lessThan(0)) || (!isNaN(average_val) && average_val < 0)) {
            const now = Date.now();
