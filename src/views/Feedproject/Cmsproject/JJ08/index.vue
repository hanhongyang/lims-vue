                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>样液号</td>
                      <td>试样质量 m（g）</td>
                      <td>消耗 EDTA 标准液的体积 V<sub>2</sub>(mL)</td>
                      <td>Ca 含量 （%）</td>
                      <td>平均值（%）</td>
                      <td>相对相差（%）</td>
                      <td>鲜样中Ca 含量 （%）</td>
                    ...
                    <template v-for="(item, index) in Details.infoList">
                      ...
                      <tr :key="`row2-${index}`">
                        ...
                        <td><el-input v-model="item.compareV2" inputmode="decimal"
                            ...
                            :readonly="Details.status == '2' || isJCRY"></el-input></td>
                        <td><el-input :value="computeCompareX(item)" readonly @blur="updateCompareValues(item)"
                            :readonly="Details.status == '2' || isJCRY"></el-input>
                        </td>
                      </tr>
                    </template>
                      ...
                      <td colspan="10">
                        <div class="formula-container">
                          ...
                          （试样分解液的总体积V<span class="math-sub">0</span>=<el-input v-model="Details.v0" inputmode="decimal"
                            @input="onNumberInput($event, Details, 'v0')" style="width:70px"
                            ...
                            @input="onNumberInput($event, Details, 'v1')" style="width:50px"
                            :readonly="Details.status == '2' || isJCRY"></el-input>ml）
                          <span style="margin-left: 200px;">
                            EDTA标准滴定溶液批号<el-input v-model="Details.edtabzddryph" inputmode="decimal" style="width:100px"
                              :readonly="Details.status == '2' || isJCRY"></el-input>
                            对钙的滴定度T<el-input v-model="Details.dgdddd" inputmode="decimal"
                              @input="onNumberInput($event, Details, 'dgdddd')" style="width:100px"
                              :readonly="Details.status == '2' || isJCRY"></el-input>×10<sup>-6</sup>g/mL
                          </span>
                    ...
                    <tr>
                      ...
                      <td colspan="5" style="text-align:left">含钙量 10%以上，允许相对相差≤3%;含钙量在 5%~10%时，允许相对相差≤5%;
                        含钙量 1%~5%时，允许相对相差≤9%;含量在1%以下，允许相对相差≤18%。</td>
                      <td colspan="4" style="text-align:left">样品前处理方法为干法,
                        箱式电炉<el-input v-model="oneBeforeTime" updateDetails style="width:50px"
                          :readonly="Details.status == '2' || isJCRY"></el-input>点~<el-input v-model="oneAfterTime"
                          updateDetails style="width:50px"
                          ...
                          updateDetails style="width:50px"
                          :readonly="Details.status == '2' || isJCRY"></el-input>点。滴定管编号<el-input
                          v-model="Details.ddgbh" inputmode="decimal" style="width:100px"
                          :readonly="Details.status == '2' || isJCRY"></el-input>
                      </td>
                    </tr>
...
import csfhyMixinthree from "@/views/Feedproject/csfhyMixinthree";
export default {
  name: "CalciumEDTA",
  mixins: [csfhyMixinthree],
  components: {
  ...
  data() {
    return {
      ...
      // 本地缓存的 key
      LOCAL_STORAGE_KEY: 'CalciumEDTA_DefaultValues',
    };
  },
  created() {
  ...
  computed: {
    ...
    formulaTipsList() {
      ...
      (this.Details.infoList || []).forEach((row, idx) => {
        ...
        if (!row.v2 || !row.compareV2) rowEmpty.push('消耗EDTA标准液的体积 V₂');
    ...
    formulaTips() {
      ...
      if (!this.Details.v1) empty.push('移动试样液体积');
      if (!this.Details.dgdddd) empty.push('对钙的滴定度');
  ...
  mounted() {
    ...
    if (this.$route.params.taskInfo) {
      ...
      // ★★★ 新增：新建时从本地加载默认值 ★★★
      this.$nextTick(() => {
        this.loadDefaultValuesFromLocal();
      });
  ...
  * 保存默认值到本地存储
  */
    saveDefaultValuesToLocal() {
      const defaultValues = {
        edtabzddryph: this.Details.edtabzddryph || '',    // EDTA标准滴定溶液批号
        dgdddd: this.Details.dgdddd || '',                 // 对钙的滴定度T
        oneBeforeTime: this.oneBeforeTime || '',           // 箱式电炉时间1-开始
        oneAfterTime: this.oneAfterTime || '',             // 箱式电炉时间1-结束
        twoBeforeTime: this.twoBeforeTime || '',           // 箱式电炉时间2-开始
        twoAfterTime: this.twoAfterTime || '',             // 箱式电炉时间2-结束
        ddgbh: this.Details.ddgbh || '',                   // 滴定管编号
      };

      try {
        localStorage.setItem(this.LOCAL_STORAGE_KEY, JSON.stringify(defaultValues));
        console.log('默认值已保存到本地:', defaultValues);
      } catch (error) {
        console.error('保存默认值到本地失败:', error);
      }
    },
     ...
     * 从本地存储读取默认值
     */
    loadDefaultValuesFromLocal() {
      try {
        const stored = localStorage.getItem(this.LOCAL_STORAGE_KEY);
        if (stored) {
          const defaultValues = JSON.parse(stored);
          console.log('从本地读取默认值:', defaultValues);

          // 填充到表单（只填充空值，不覆盖已有数据）
          if (!this.Details.edtabzddryph && defaultValues.edtabzddryph) {
            this.$set(this.Details, 'edtabzddryph', defaultValues.edtabzddryph);
          }
          if (!this.Details.dgdddd && defaultValues.dgdddd) {
            this.$set(this.Details, 'dgdddd', defaultValues.dgdddd);
          }
          if (!this.oneBeforeTime && defaultValues.oneBeforeTime) {
            this.oneBeforeTime = defaultValues.oneBeforeTime;
          }
          if (!this.oneAfterTime && defaultValues.oneAfterTime) {
            this.oneAfterTime = defaultValues.oneAfterTime;
          }
          if (!this.twoBeforeTime && defaultValues.twoBeforeTime) {
            this.twoBeforeTime = defaultValues.twoBeforeTime;
          }
          if (!this.twoAfterTime && defaultValues.twoAfterTime) {
            this.twoAfterTime = defaultValues.twoAfterTime;
          }
          if (!this.Details.ddgbh && defaultValues.ddgbh) {
            this.$set(this.Details, 'ddgbh', defaultValues.ddgbh);
          }
    ...
    validateEmpty() {
      ...
      (this.Details.infoList || []).forEach((row, idx) => {
        ...
        if (!row.v2) empty.push(`第${idx + 1}行消耗EDTA标准液的体积 V₂`);
        if (!row.compareM) empty.push(`第${idx + 1}行对比试样质量 m`);
        if (!row.compareV2) empty.push(`第${idx + 1}行对比消耗EDTA标准液的体积 V₂`);
      });
    ...
    computex(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [item.v2, this.Details.dgdddd, this.Details.v0, item.m, this.Details.v1];
      if (!this.hasRequiredValues(requiredValues)) {
        this.$set(item, 'ca', ''); // *** 逻辑修改：确保清空 ***
        return '';
      }

      try {
        const v2 = new Decimal(item.v2);
        const T = new Decimal(this.Details.dgdddd).times(1e-6);
        const v0 = new Decimal(this.Details.v0);
        const m = new Decimal(item.m);
        const v1 = new Decimal(this.Details.v1);


        // 检查分母是否为0
        if (m.times(v1).equals(0)) { // *** 逻辑修改：检查 m * v1 ***
          this.$set(item, 'ca', '0'); // *** 逻辑修改：确保赋值 ***
          return '0';
        }

        // 计算公式: (T * v2 * v0) / (m * v1) * 100
        const numerator = T.times(v2).times(v0);
        const result = numerator.dividedBy(m.times(v1)).times(100);

        // const formattedResult = this.formatPrecision(result);
        const formattedResult = this.formatBankerRounding(result, 4);
        this.$set(item, 'ca', formattedResult); // *** 逻辑修改：确保保存 ***
    ...
    computeCompareX(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [item.compareV2, this.Details.dgdddd, this.Details.v0, item.compareM, this.Details.v1];
      if (!this.hasRequiredValues(requiredValues)) {
        this.$set(item, 'compareCa', ''); // *** 逻辑修改：确保清空 ***
        return '';
      }
      ...
      try {
        ...
        // 检查分母是否为0
        if (m.times(v1).equals(0)) { // *** 逻辑修改：检查 m * v1 ***
          this.$set(item, 'compareCa', '0'); // *** 逻辑修改：确保赋值 ***
          return '0';
        }
        ...
        // const formattedResult = this.formatPrecision(result);
        const formattedResult = this.formatBankerRounding(result, 4);
        this.$set(item, 'compareCa', formattedResult); // *** 逻辑修改：确保保存 ***
        return formattedResult;
      } catch (error) {
        console.error('计算对比X值错误:', error);
        this.$set(item, 'compareCa', '');
        return '';
      }
    },
    // 精度格式化方法
    formatPrecision(value) {
      ...
      /* ---------- 特殊有效数字字段 ---------- */
      if (field === 'ca') return num.toSignificantDigits(4).toString(); // 4 位有效数字
      if (field === 'average') return num.toSignificantDigits(3).toString(); // 3 位有效数字
      if (field === 'xyzhl') return num.toSignificantDigits(3).toString(); // 3 位有效数字
      // 优先使用小数位，其次使用有效数字位
    ...
    updateCompareValues(item) {
      item.compareCa = this.computeCompareX(item);
      item.ca = this.computex(item);

      this.$nextTick(() => {
        const ca = item.ca; // *** 逻辑修改：使用已存值 ***
        const compareCa = item.compareCa;

        if (ca !== '' && compareCa !== '' && ca !== null && compareCa !== null) {
          const avg = new Decimal(ca).plus(new Decimal(compareCa)).dividedBy(2);
          const absoluteDiff = Decimal.abs(new Decimal(ca).minus(new Decimal(compareCa)));
          // *** 逻辑修改：使用 avg 计算相对相差 ***
          ...
          if ((new Decimal(ca).lessThan(0)) || (new Decimal(compareCa).lessThan(0)) || (!isNaN(average_val) && average_val < 0)) {
            const now = Date.now();
