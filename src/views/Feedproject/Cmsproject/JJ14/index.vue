        ...
        <table cellpadding="0" cellspacing="0" width="100%" id="exportTable" style="text-align: center;">
          <tbody>
            ...
            <tr>
              ...
              <td colspan="6" style="text-align:left">丙酮<el-input v-model="sjhphOne" style="width:130px"
                  :readonly="Details.status == '2' || isJCRY"></el-input>硫酸溶液<el-input v-model="sjhphTwo"
                  style="width:130px" :readonly="Details.status == '2' || isJCRY"></el-input>氢氧化钾溶液<el-input
                  v-model="sjhphThree" style="width:130px" :readonly="Details.status == '2' || isJCRY"></el-input></td>
            </tr>
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="text-align: center;">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>滤袋质量 m₁（g）</td>
                      <td>试样质量 m（g）</td>
                      <td>坩埚+袋+试样残渣烘干后总质量 m<sub>2</sub>(g)</td>
                      <td>坩埚+试样残渣灰化后总质量 m<sub>3</sub>(g)(g)</td>
                      <td>CF含量x(%)</td>
                      <td>平均值(%)</td>
                      <td>相对相差/绝对偏差(%)</td>
                      <td>鲜样中CF含量(%)</td>
                        ...
                        <div class="formula-container">
                          ...
                          <div class="formula-note" style="margin-top: 6px; font-size: 14px; white-space: normal;">
                            ...
                            <span style="margin-left:10px">
                              (m₀=处理前滤袋质量<el-input v-model="Details.clqldzl" inputmode="decimal" style="width:120px"
                                @input="onNumberInput($event, Details, 'clqldzl')"
                                :readonly="Details.status == '2' || isJCRY"></el-input>g，
                              m<sub>A</sub>=干燥后滤袋+坩埚质量<el-input v-model="Details.gzhldggzl" inputmode="decimal"
                                style="width:120px" @input="onNumberInput($event, Details, 'gzhldggzl')"></el-input>g，
                              m<sub>B</sub>=灰化后滤袋+坩埚质量<el-input v-model="Details.hhhldggzl" inputmode="decimal"
                                style="width:120px" @input="onNumberInput($event, Details, 'hhhldggzl')"></el-input>g，
                              校正因子C₁<el-input :value="computeC1(item)" style="width:120px" readonly
                                @blur="updateCompareValues(item)"></el-input>)
                            </span>
                          </div>
                        </div>
                    ...
                    <tr>
                      ...
                      <td colspan="10" style="text-align:left">CF 含量在&LT;5%时，绝对差值≤0.6%;CF 含量在 5%-10%时，相对相差≤10%; CF
                        含量在&GT;
                        10%时，相对相差≤6%。</td>
                    </tr>
  ...
  computed: {
    ...
    formulaTipsList() {
      ...
      (this.Details.infoList || []).forEach((row, idx) => {
        const rowEmpty = [];
        if (!row.m2 || !row.compareM2) rowEmpty.push('烘干后总质量 m₂');
        if (!row.m1 || !row.compareM1) rowEmpty.push('滤袋质量 m₁');
        if (!row.m3 || !row.compareM3) rowEmpty.push('灰化后总质量 m₃');
    ...
    formulaTips() {
      const empty = [];
      if (!this.Details.clqldzl) empty.push('处理前滤袋质量');
      if (!this.Details.gzhldggzl) empty.push('干燥后质量');
      if (!this.Details.hhhldggzl) empty.push('灰化后质量');
  ...
  methods: {
    ...
    validateEmpty() {
      ...
      (this.Details.infoList || []).forEach((row, idx) => {
        ...
        if (!row.m2) empty.push(`第${idx + 1}行烘干后总质量 m₂`);
        if (!row.m3) empty.push(`第${idx + 1}行灰化后总质量 m₃`);
        if (!row.compareM1) empty.push(`第${idx + 1}行对比滤袋质量 m₁`);
        if (!row.compareM) empty.push(`第${idx + 1}行对比试样质量 m`);
        if (!row.compareM2) empty.push(`第${idx + 1}行对比烘干后总质量 m₂`);
        if (!row.compareM3) empty.push(`第${idx + 1}行对比灰化后总质量 m₃`);
      });
    ...
    updateDetails() {
      this.Details.sjhph = `${this.sjhphOne},${this.sjhphTwo},${this.sjhphThree}`;
    ...
    onNumberInput(val, row, key) {
      ...
      // *** 逻辑修改：全局字段需要特殊处理 ***
      if (key === 'clqldzl' || key === 'gzhldggzl' || key === 'hhhldggzl') {
    ...
    computex(item) {
      ...
      // 检查必需的值是否都已输入
      const requiredValues = [item.m1, item.m, item.m2, item.m3];
      ...
      try {
        ...
        const numerator = m2.minus(m3)
        const numerator2 = m1.times(C1)
        const numerator1 = numerator.minus(numerator2)
        const result = numerator1.dividedBy(m).times(100);
    ...
    computeCompareX(item) {
      ...
      const C1 = new Decimal(c1Raw);
      // 检查必需的值是否都已输入
      const requiredValues = [item.compareM1, item.compareM, item.compareM2, item.compareM3];
      if (!this.hasRequiredValues(requiredValues)) {
    ...
    computeC1(item) {
      // 检查必需的值是否都已输入
      const requiredValues = [this.Details.clqldzl, this.Details.gzhldggzl, this.Details.hhhldggzl];
      if (!this.hasRequiredValues(requiredValues)) {
        return '';
      }
      try {
        const q = new Decimal(this.Details.clqldzl);
        const h = new Decimal(this.Details.gzhldggzl);
        const h1 = new Decimal(this.Details.hhhldggzl);
        ...
        const num1 = h.minus(h1)
        const result = num1.dividedBy(q);
        const four = result.toDecimalPlaces(4).toString();
        console.log('C1原始值', four);   // 看这里是不是 4 位
        this.Details.jzyz = four;       // 整单校正因子
        return four;
    ...
    formatPrecision(value, field = '') {
      ...
      if (field === 'average' || field === 'absoluteDifference') {
        // 平均值 / 相对相差：1 位小数，强制补 0
        return num.toFixed(1);
      }
      /* **** 新增：鲜样中 CF 含量 **** */
      if (field === 'xyzhl') {
        // 鲜样中 CF 含量：3 位小数，强制补 0
    ...
    updateCompareValues(item) {
      ...
      this.$nextTick(() => {
        ...
        if (x !== '' && compareX !== '' && x !== null && compareX !== null) {
          ...
          this.$set(item, 'absoluteDifference', this.formatPrecision(relativeDiff, 'absoluteDifference')); // (relativeDiff.toFixed(2) 是原始逻辑，予以保留)
          ...
          this.$set(item, 'xyzhl', this.formatPrecision(xyzhlVal, 'xyzhl'));
        } else {
