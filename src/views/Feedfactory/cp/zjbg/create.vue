  ...
  methods: {
    ...
    handleSave(isChained = false) {
      ...
      return new Promise((resolve, reject) => {
        ...
        console.log(submitData, '传给后端')

        let apiCall;
        const isUpdate = !!this.form.opSamplingPlanId;

        if (isUpdate) {
          apiCall = updatePlan(submitData);
        } else {
        ...
        this.$nextTick(() => {
          if (this.testItemList && this.testItemList.length > 0) {
            this.testItemList.forEach(row => {
              // 如果 Map 里有这个 ID，就勾选
              // 注意：这里用 row.labtestItemsId 来匹配 Map 的 key
    ...
    handleTestItemSelectionChange(selection) {
      ...
      const currentPageIds = this.testItemList.map(item => item.labtestItemsId);

      // 2. 找出当前被选中的 ID 集合
      const selectedIds = selection.map(item => item.labtestItemsId);

      // 3. 遍历当前页的每一行数据，决定是 添加 还是 删除
      this.testItemList.forEach(row => {
        const rowId = row.labtestItemsId;
    ...
    confirmTestItems() {
      if (this.currentSampleIndex !== -1) {
        ...
        this.form.opSamplingPlanSampleList[this.currentSampleIndex].opSamplingPlanItemList = selectedItems.map(item => ({
          // 【绝对关键】必须同时兼容 itemId 和 labtestItemsId
