  ...
  <div class="app-container">
      ...
      <div class="form-section">
        <table class="info-table">
          ...
          <tr>
            ...
            <td>委外厂家</td>
            <td>
              <el-select v-model="form.outsourcedFactory" placeholder="请选择委外厂家" style="width: 100%;">
                <el-option v-for="dict in dict.type.outsourced_factory" :key="dict.value" :label="dict.label"
                  :value="dict.value">
                </el-option>
              </el-select>
              <!-- <el-input v-model="form.outsourcedFactory"></el-input> -->
            </td>
          </tr>
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body
      ...
      <el-upload ref="upload" :action="upload.Url" :headers="upload.headers" :file-list="carReportFileList"
        :on-success="handleCarReportSuccess" :on-remove="handleCarReportRemove" list-type="picture">
...
export default {
  ...
  computed: {
    ...
    carReportFileList() {
      ...
      // id → 预览地址（临时）
      return this.form.carFileId.split(',').map(id => ({
        name: `报告${id.slice(-4)}`,   // 随便起个名字
        url: `${process.env.VUE_APP_BASE_API}/file/preview/${id}`,  // 临时地址
        response: { data: id }        // 关键：给 on-remove 用
      }));
    }
  },
  created() {
  ...
  methods: {
    ...
    handleCarReportSuccess(res, file) {
      if (res.code === 200 && res.data) {
        ...
        // 记录在 form 中
        const ids = this.form.carFileId ? this.form.carFileId.split(',') : [];
        ...
        // 关键：把 fileId 挂到 file 对象上，删除时可以直接用
        file.fileId = fileId;
        // 可选：也把 fileUrl 挂上
    ...
    handleCarReportRemove(file) {
      // 1. 兼容各种情况，尽量拿到真正的 fileId
      let removeId = null;

      const resp = file.response && file.response.data;

      if (file.fileId) {
        // 推荐：优先用我们在 on-success 里挂上的 file.fileId
        removeId = file.fileId;
      } else if (resp && typeof resp === 'object') {
        // 新上传：response.data = { fileId, fileUrl }
        removeId = resp.fileId || resp.id;
      } else {
        // 回显旧数据：response.data = 'xxx'
        removeId = resp;
      }

      if (!removeId) {
        console.warn('未能从 file 中解析到要删除的 fileId：', file);
        return;
      }
      ...
      // 2. 更新 carFileId 字符串
      const idArr = (this.form.carFileId || '')
        .split(',')
        .filter(Boolean);

      const urlArr = (this.form.carFileUrl || '')
        .split(',')
        .filter(Boolean);

      const idx = idArr.indexOf(removeId);
      if (idx > -1) {
        idArr.splice(idx, 1);
        // 如果 fileUrl 和 fileId 一一对应，也一并删掉对应下标
        if (urlArr[idx]) {
          urlArr.splice(idx, 1);
        }
      }
      ...
      // 调试用
      console.log('删除后 this.form.carFileId =', this.form.carFileId);
    },
    dqyzf() {
    ...
    viewReport(row, index) {
      ...
      this.tempUnquality = {
        ...
        ggQualityFileIds: row.ggQualityFileIds || '' // 注意字段名是 ggQualityFileIds
      };

      // 如果有图片文件ID，需要初始化上传组件显示
      if (this.tempUnquality.ggQualityFileIds && this.tempUnquality.ggQualityFileIds.length > 0) {
        this.initSensoryFileList(this.tempUnquality.ggQualityFileIds);
      } else {
    ...
    openTestItemDialog(index) {
      ...
      if (currentSample && currentSample.opSamplingPlanItemList) {
        currentSample.opSamplingPlanItemList.forEach(item => {
          const rawId = item.itemId || item.labtestItemsId;
          if (rawId) {
            const idStr = String(rawId);
            // ★★★ 核心修改：直接存入整个 item 对象，并强制修正 itemId
            this.allSelectedTestItems.set(idStr, {
              ...item,
              itemId: idStr // 确保 ID 格式统一
            });
          }
        });
      }
    ...
    getTestItemList() {
      ...
      getItemByInvbillCode(this.testItemQuery).then(response => {
        ...
        this.testItemList = response.rows.map(item => {
          ...
          // 2. 确保逻辑用的 itemId 存在且为字符串 (兼容处理)
          // 如果后端返回的是 labtestItemsId 或 id，统一赋值给 itemId 供前端逻辑使用
          fullItem.itemId = String(item.itemId || item.labtestItemsId || item.id);
