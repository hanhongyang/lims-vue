                    ...
                    <el-table :data="form.opTestResultInfoList">
                        ...
                        <el-table-column :label="getTestTypeLabel" prop="result">
                            <template slot-scope="scope">
                                ...
                                <div v-if="scope.row.opSamplingPlanItem.qualitativeOrQuantitative != null">
                        ...
                        <el-table-column label="判定" prop="pdjg">
                            <template slot-scope="scope">
                                <!-- 定性：人工输入判定 -->
                                <el-input v-if="isQualitative(scope.row)" v-model="scope.row.pdjg" placeholder="判定结果"
                                    :disabled="form.status !== '2'" />

                                <!-- 定量：自动合格/不合格 -->
                                <el-tag v-else :type="autoCheckResult(scope.row).type">
                                    {{ autoCheckResult(scope.row).text }}
                                </el-tag>
                            </template>
                        </el-table-column>
...
export default {
    ...
    methods: {
        /* 是否为定性 */
        isQualitative(row) {
            return row.opSamplingPlanItem?.qualitativeOrQuantitative === '1';
        },

        /* 定性选择后自动填判定 */
        onQualitativeChange(row) {
            row.pdjg = row.result;   // 需要可自行扩展
        },

        /* 定量输入后自动计算合格/不合格 */
        onQuantitativeInput(row) {
            const num = Number(row.result);
            const item = row.opSamplingPlanItem;
            if (!item || isNaN(num)) return;

            const lower = item.lowerLimit == null ? -Infinity : Number(item.lowerLimit);
            const upper = item.upperLimit == null ? Infinity : Number(item.upperLimit);

            row.checkResult = num >= lower && num <= upper ? '1' : '2';
        },

        /* 供 el-tag 显示用 */
        autoCheckResult(row) {
            // 定量无值时直接返回 --
            if (!row.result || row.result === '') return { text: '--', type: 'info' };
            const map = {
                '1': { text: '合格', type: 'success' },
                '2': { text: '不合格', type: 'danger' },
                '3': { text: '让步接收', type: 'warning' }
            };
            return map[row.checkResult] || { text: '--', type: 'info' };
        },
        ...
        submitForm(action) {
            this.$refs.form.validate(valid => {
                ...
                /* =====  仅对 checkResult 做“空值不传”处理  ===== */
                this.form.opTestResultInfoList.forEach(item => {
                    // 0 保持原有值（0->让步接收'3'，1->合格'1'，2->不合格'2'）
                    if (item.result === '' || item.result == null) {
                        item.checkResult = undefined;   // 空值：后端收不到该字段
                    }
                    // 其余情况 item.checkResult 已正确，不再动
                });

                /* 下面是你原来的保存/提交逻辑，无任何改动 */
                const api = action === 'save' ? updateTestResult : submitTestResult;
                api(this.form).then(() => {
                    this.$modal.success(action === 'save' ? '保存成功' : '提交成功');
                    this.open = false;
                    this.getList();
                    this.getTabCounts();
                });
