...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        bloodTaskItemType: null, // 检测项目 (用于 tab 0)
  ...
  methods: {
    ...
    resetQuery() {
      ...
      this.queryParams.bloodTaskItemType = null;
    ...
    handleSelectionChange(selection) {
      // 这里的 selection 是我们传入的 orderedSelectedRows (带顺序)

      // 1. 基础状态更新
      this.ids = selection.map(item => item.entrustOrderId);
      this.orderIds = selection.map(item => item.opBloodEntrustOrderId);
      ...
      if (selection.length > 1) {
        ...
        const firstType = String(selection[0].bloodTaskItemType || '');

        // 查找是否存在与第一项不一致的数据
        const hasDiff = selection.some(item => {
          const currentType = String(item.bloodTaskItemType || '');
          return currentType !== firstType;
        });

        if (hasDiff) {
          this.isSame = false;
          // 如果是在 '待化验' tab，且发生了冲突 (通常会被 handleRowSelect 拦截，这里作为双重保险)
          if (this.activeTab === '0') {
            this.$modal.msgWarning('检测到您勾选了不同的检测项目！请确保所有选中的任务项目一致。');
          }
        } else {
          this.isSame = true;
        }
    ...
    handleRowSelect(selection, row) {
      ...
      if (isSelected) {
        ...
        if (this.orderedSelectedRows.length > 0) {
          // 取出已选列表的第一个作为基准
          const baseType = String(this.orderedSelectedRows[0].bloodTaskItemType || '');
          const currentType = String(row.bloodTaskItemType || '');

          // 如果不一致，且都在 '待化验' 状态
          if (baseType !== currentType && this.activeTab === '0') {
            const baseName = this.selectDictLabel(this.dict.type.blood_task_item_type, this.orderedSelectedRows[0].bloodTaskItemType);
            const currentName = this.selectDictLabel(this.dict.type.blood_task_item_type, row.bloodTaskItemType);

            this.$message.warning(`不能选择该行！它属于[${currentName}]，而您已选的项目是[${baseName}]`);

            // 关键：让表格在视觉上取消这一行的勾选
            this.$refs.tablePending.toggleRowSelection(row, false);
            return; // 阻断执行，不加入 orderedSelectedRows
          }
        }
      ...
      // 2. 手动调用 handleSelectionChange 更新状态
