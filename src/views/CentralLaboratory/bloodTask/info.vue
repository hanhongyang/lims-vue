        ...
        <div style="padding-top: 40px">
            <el-form ref="form" :model="form" label-width="120px">
                ...
                <el-card class="test-config-card" v-for="table in groupedTables" :key="table.type"
                    ...
                    <div v-if="bloodTaskItemType == '8' && groupedTables.length > 0"
                        style="margin-top: 15px; padding: 12px 15px; background-color: #f8f9fa; border-radius: 4px; color: #606266; font-size: 14px; line-height: 1.5;">
                        <span style="font-weight: bold; margin-right: 8px;">备注:</span>
                        <span>{{ groupedTables[0].data[0].testRemark }}</span>
                        <!-- <span>1111</span> -->
                    </div>
                    <el-table :data="table.data" border stripe style="width: 100%" v-loading="loading" height="65vh">
...
export default {
    ...
    methods: {
        ...
        getColumnsForType(bloodType, data) {
            // 1. 固定的前三列 (所有类型都显示)
            const commonColumns = [
                { prop: 'entrustDeptName', label: '委托单位', width: 130, editable: false },
                { prop: 'gh', label: '管号', width: 130, editable: false },
                { prop: 'sampleName', label: '牛号', width: 130, editable: false },
            ];
            ...
            if (bloodType === '0') {
                //如果是口蹄疫，添加 A型 和 O型 批号
                batchColumns = [
                    { prop: 'aSjph', label: 'A型试剂批号', width: 160, editable: false },
                    { prop: 'oSjph', label: 'O型试剂批号', width: 160, editable: false }
                ];
            } else if (bloodType === '9' || bloodType === '8') {
                // 如果是早孕或生化，不显示批号
                batchColumns = [];
            } else {
                // 其他类型，显示通用试剂批号
                batchColumns = [
                    { prop: 'sjph', label: '试剂批号', width: 180, editable: false }
                ];
            }

            // 4. 返回合并后的列
            return [...commonColumns, ...filteredDynamicColumns, ...batchColumns];
        },
