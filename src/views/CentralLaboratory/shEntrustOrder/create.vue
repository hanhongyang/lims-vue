    ...
    <div class="entrust-form">
          ...
          <tr>
            ...
            <td colspan="5">
              <div class="remarks">
                <!-- <p style="display: flex; align-items: center; margin: 8px 0; position: relative;">
                  <el-radio-group v-model="form.itemCode" :class="{ 'required-field': !form.itemCode }">
                    <el-radio v-for="dict in dict.type.blood_task_item_type" :key="dict.value" :label="dict.value"  
                      @click.native.prevent="toggleRadio('itemCode', dict.value)" :disabled="isZS">{{
                        dict.label }}</el-radio>
                  </el-radio-group>
                ...
                <p style="display: flex; align-items: center; margin: 8px 0;"      
                  ...
                           <el-radio-group v-model="form.itemTy" size="mini">
                    ...
                         <el-radio v-for="dict in dict.type.fmd_time" :key="dict.value" :label="dict.value" border
                             @click.native.prevent="toggleRadio('itemTy', dict.value)" :disabled="isZS">{{
                        dict.label }}</el-radio>
                ...
                <p style="display: flex; align-items: center; margin: 8px 0;"
                  ...
                  <el-radio-group v-model="form.itemBbkt" size="mini">
                    ...
                    <el-radio v-for="dict in dict.type.bbktjc_type" :key="dict.value" :label="dict.value" border
                      @click.native.prevent="toggleRadio('itemBbkt', dict.value)" :disabled="isZS">{{
                ...
                <p style="display: flex; align-items: center; margin: 8px 0;"
                  v-if="!isZaoyunSelected && !isShenghuaSelected"><span style="min-width: 120px;">免疫时间(免疫后的样品)</span>
                  <el-input v-model="form.immunityTime" placeholder="请输入免疫时间" style="width: 200px;"
      ...
      <div class="form-section">
        <el-table :data="form.sampleList" border style="width: 100%" height="30vh">
          ...
          <el-table-column label="牛号" width="300" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column label="管号" width="300" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column label="类别" width="300" align="center" v-if="!isZaoyunSelected && !isShenghuaSelected">
            <template slot-scope="scope">
          ...
          <el-table-column label="配种天数" width="300" align="center" v-if="isZaoyunSelected">
            <template slot-scope="scope">
              <el-input v-model="scope.row.pzts" placeholder="请输入配种天数"></el-input>
            </template>
          </el-table-column>

          <el-table-column label="备注" width="300" align="center" v-if="!isZaoyunSelected">
            <template slot-scope="scope">
      ...
      <div class="form-actions">
        ...
        <el-button @click="viewReport" type="success" icon="el-icon-printer">查看报告</el-button>
...
export default {
  ...
  methods: {
    ...
    goBack() {
      const obj = { path: "/CentralLaboratory/bloodEntrustOrder" };
    ...
    jsForm() {
      ...
      updateBloodEntrustOrder(submitData).then(response => {
        ...
        this.$tab.closePage();
        this.$router.push({ path: '/CentralLaboratory/sampleReceive/SH' });
      ...
      reader.onload = (e) => {
        try {
          ...
          if (processingResult.errors.length > 0) {
            ...
            // 4. 添加数据：覆盖已有数据，并为每条样品添加 sequence
    ...
    processExcelData(dataRows, headers) {
      ...
      dataRows.forEach((row, index) => {
        ...
        const sample = {
          sampleName: '', gh: '', sampleType: null, pzts: '', remark: ''
        };
        let hasData = false;
        ...
        // 过滤 (您要求的)：牛号和管号必须都有值
        if (!sample.sampleName || !sample.gh) {
          if (hasData) {
            //（跳过第4、5行）
            console.log(`第 ${rowNum} 行因缺少牛号或管号被跳过。`);
          }
          return;
        }
