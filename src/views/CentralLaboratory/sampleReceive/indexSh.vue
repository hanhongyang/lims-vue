<template>
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="sampleReceiveList" height="75%" style="max-height:75%;overflow-y: auto;"
            ...
            <el-table-column label="样品数量" align="center" prop="sampleAmount" width="100" />
            ...
            <el-table-column label="委托单位" align="center" prop="deptName" />
            <!-- <el-table-column label="检测项目" align="center" prop="itemName" width="300">
                <template slot-scope="scope">
                    <dict-tag :options="dict.type.biochemistry_item_type" :value="scope.row.itemName" />
                </template>
            </el-table-column> -->
...
export default {
    name: "SampleReceive",
    dicts: ['entrust_order_status', 'sample_receive_type', 'feed_test_method', 'biochemistry_item_type'],
    data() {
        return {
    ...
    methods: {
        ...
        dyyl() {
            ...
            if (unReceivedData.length > 0) {
                const names = unReceivedData.map(item => item.sampleName).join('、');
                errorMessages.push(`未接收样品（${names}）不可打印`);
            }
