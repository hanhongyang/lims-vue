        ...
        <div class="sta-main">
            <div class="page">
                <div id="captureArea" class="capture-area">
                    ...
                    <div class="form-title">GMMY/ZJ-29-14 生化检测送检单</div>
                    <!-- 表头信息 -->
                    <table class="info-table" style="margin-top: 60px;">
                        ...
                        <tr>
                            <td>送检单位</td>
                            <td>{{ Details.entrustDeptName }}</td>
                            <td>送样日期</td>
                            <td>{{ Details.sendSampleDate }}</td>
                            <td>送样人/电话</td>
                            <td>{{ Details.sendSampleUserName }} / {{ Details.entrustContactPhone }}</td>
                        </tr>
                        <tr>
                            <td>地址</td>
                            <td>{{ Details.address }}</td>
                            <td>报告接收邮箱</td>
                            <td>{{ Details.entrustContactEmail }}</td>
                            <td>接收人/接收日期</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>检测项目</td>
                            <td colspan="5">{{ biochemistryLabels }}</td>
                        </tr>
                    </table>
...
export default {
    dicts: ['pcr_task_item_type', 'allows', 'report_receive_type', 'sample_return_policy', 'sample_packaging', 'storage_requirement', 'sample_state', 'testing_service_level', 'feed_test_method', 'biochemistry_item_type'],
    data() {
        return {
            // 遮罩层
            loading: false,
    ...
    mounted() {
        ...
        this.biochemistryItemType = src.biochemistryItemType || [];
        ...
        // ... 数据赋值后添加
        console.log('全局检测项目编码：', this.biochemistryItemType);
        console.log('检测项目字典：', this.biochemistryMap);
        console.log('最终显示的检测项目：', this.biochemistryLabels);
    },
    computed: {
        // 把字典先转 map，方便后面多次查
        biochemistryMap() {
            const map = {};
            (this.dict.type.biochemistry_item_type || []).forEach(d => {
                map[d.value] = d.label;
            });
            return map;
        },

        // 修正：从全局 biochemistryItemType 取检测项目，而非每个样品
        biochemistryLabels() {
            // 情况1：biochemistryItemType 是编码数组（如 ["1", "2"]）
            if (Array.isArray(this.biochemistryItemType) && this.biochemistryItemType.length > 0) {
                return this.biochemistryItemType
                    .map(code => this.biochemistryMap[String(code)] || code) // 编码转中文
                    .join('，');
            }
            // 情况2：biochemistryItemType 是直接的中文字符串（兼容可能的字段格式）
            else if (typeof this.biochemistryItemType === 'string' && this.biochemistryItemType) {
                return this.biochemistryItemType;
            }
            // 情况3：无检测项目时显示提示
            return '无';
