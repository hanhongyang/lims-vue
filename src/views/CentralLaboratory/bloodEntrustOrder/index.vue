<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="bloodEntrustOrderList" height="70vh">
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ path: '/CentralLaboratory/BloodEntrustOrderCreate', query: { type: 'add' } }).catch(err => {
        if (err.name !== 'NavigationDuplicated') {
          console.error('路由跳转错误:', err);
        }
      });
      // this.$router.push('/CentralLaboratory/BloodEntrustOrderCreate').catch(err => {
      //   if (err.name !== 'NavigationDuplicated') {
      //     console.error('路由跳转错误:', err);
      //   }
      // });
    ...
    handleUpdate(row) {
      ...
      this.$router.push({ path: '/CentralLaboratory/BloodEntrustOrderCreate', query: { id: id } });
    },
