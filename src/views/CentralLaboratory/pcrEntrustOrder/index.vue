<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="pcrEntrustOrderList" height="70vh">
...
<script>
import { listPcrEntrustOrder, getPcrEntrustOrder, delPcrEntrustOrder, addPcrEntrustOrder, updatePcrEntrustOrder } from "@/api/CentralLaboratory/pcrEntrustOrder";

export default {
  name: "PcrEntrustOrder",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPcrEntrustOrder(this.queryParams).then(response => {
        this.pcrEntrustOrderList = response.rows;
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push('/CentralLaboratory/PcrEntrustOrderCreate').catch(err => {
    ...
    /** 修改按钮操作 */
    handleUpdate(row) {
      const id = row.opPcrEntrustOrderId || this.ids
      this.$router.push({ path: '/CentralLaboratory/PcrEntrustOrderCreate', query: { id: id } });
    },

    /** 导出按钮操作 */
    handleExport() {
      this.download('labtest/pcrEntrustOrder/export', {
        ...this.queryParams
      }, `pcrEntrustOrder_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
