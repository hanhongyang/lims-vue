    ...
    <el-tabs v-model="activeTab" @tab-click="handleTabClick">
      ...
      <el-tab-pane label="待审核" name="2">
        <el-form :model="queryParams2" ref="queryForm2" size="small" :inline="true" v-show="showSearch"
          ...
          <el-form-item label="导入文件名" prop="fileName">
            <el-input v-model="queryParams2.fileName" placeholder="请输入导入文件名" clearable
              @keyup.enter.native="handleQuery" />
          </el-form-item>
          <el-form-item label="导出文件名" prop="exportFileName">
            <el-input v-model="queryParams2.exportFileName" placeholder="请输入导出文件名" clearable
              @keyup.enter.native="handleQuery" />
          </el-form-item>
      ...
      <el-tab-pane label="已审核" name="3">
        ...
        <el-table v-loading="loading" :data="baseListAudited" height="64vh">
          <el-table-column type="selection" width="55" align="center" />
          <el-table-column label="导入时间" align="center" prop="testEndTime" />
          <el-table-column label="审核时间" align="center" prop="daterangeExamineDate" />
          <el-table-column label="导入人" align="center" prop="testUser" />
          <el-table-column label="导入文件名" align="center" prop="fileName" />
          <el-table-column label="导出文件名" align="center" prop="exportFileName" />
          <el-table-column label="导出条数" align="center" prop="fileSize" />
...
import { cancelExamineBloodResult } from "@/api/CentralLaboratory/sampleTest";
export default {
  name: "bloodResultBase",
  ...
  methods: {
    ...
    getInfo(row) {
      ...
      // ** 注意：如果 getInfo 跳转的页面需要 bloodTaskItemType， **
      // ** 而已审核列表的 row 中不再包含它，需要额外处理 **
      // const bloodTaskItemType = row.bloodTaskItemType; // 可能为 undefined

      // 临时方案：如果 bloodTaskItemType 缺失，可以尝试从 queryParams3 获取（如果用户已选择）
      // 或提示用户无法跳转，或修改后端让已审核列表也返回 bloodTaskItemType
      const bloodTaskItemType = row.bloodTaskItemType || this.queryParams3.bloodTaskItemType;
      if (!bloodTaskItemType && this.activeTab === '3') {
        this.$modal.msgWarning("无法确定检测项目类型，请先在查询条件中选择");
        return;
      }

      this.$router.push({
        path: '/CentralLaboratory/bloodResultInfo',
        query: {
          resultNo: resultNo,
    ...
    handleCancelExamine(row) {
      ...
      this.$modal.confirm('是否确认取消审核化验单号为"' + resultNo + '"的检测结果？').then(() => {
        return cancelExamineBloodResult({ resultNo: resultNo });
      }).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认作废化验单号为"' + resultNo + '"的检测结果？').then(() => {
        // TODO: 确认并调用作废接口
        // return deleteBloodResult(resultNo);
