  ...
  <div class="app-container">
    <el-tabs v-model="activeTab" @tab-click="handleTabClick">
        ...
        >
          ...
          <el-form-item label="检测项目" prop="bloodTaskItemType">
            <el-select v-model="queryParams.bloodTaskItemType" placeholder="请选择检测项目" clearable>
              <el-option
                v-for="dict in dict.type.blood_task_item_type"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              />
      ...
      <el-tab-pane :label="`制作中 (${tabCounts['1']})`" name="1">
        ...
        >
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button size="mini" type="text" icon="el-icon-edit" @click="handlebgdy(scope.row)">提交</el-button>
            </template>
          </el-table-column>
        </el-table>
        <pagination
      ...
      <el-tab-pane :label="`待审核 (${tabCounts['2']})`" name="2">
        ...
        >
          ...
          <!-- <el-table-column label="检测时间" align="center" prop="sendSampleDate" width="180">
            <template slot-scope="scope">
        ...
        >
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="text" icon="el-icon-edit" @click="handlebgdy(scope.row)">发送报告</el-button>
    ...
    >
      <div class="email-dialog-content">
        ...
        邮件标题：<el-input v-model="title" placeholder="请输入邮件标题" clearable />
        正文内容：<el-input v-model="content" type="textarea" placeholder="请输入正文内容" clearable />
...
export default {
  ...
  data() {
    return {
      ...
      // 遮罩层
      loading: true,
      // 选中数组，用于存储选中的 bloodEntrustOrderId
      ids: [],
      // 子表选中数据
      ...
      queryParams: {
        ...
        endReportTime: null,
        bloodTaskItemTypeNotIn: '8,9' // 排除的检测项目类型
      },
      sendList: [],
      // 邮箱弹窗相关
  ...
  created() {
    const tabName = this.$route.query.tab || '0'
  ...
  methods: {
    ...
    downloadPdf(row) {
      ...
      // const link = document.createElement('a');
      // link.href = 'http://192.168.31.242:13742/profile/upload/2025/12/16/检测报告_20251216161249A001.pdf';
      // link.download = '检测报告_20251216161249A001.pdf'; // 指定下载文件名
      // document.body.appendChild(link);
      // link.click();
      // document.body.removeChild(link);
      // const url = 'http://192.168.31.242:13742/profile/upload/2025/12/16/%E6%A3%80%E6%B5%8B%E6%8A%A5%E5%91%8A_20251216161249A001.pdf'

      // const link = document.createElement('a')
      // link.href = url
      // link.download = '检测报告.pdf'  // 设置下载文件名
      // link.target = '_blank'
      // document.body.appendChild(link)
      // link.click()
      // document.body.removeChild(link)
      // 调用下载接口
      // window.open('http://192.168.31.242:13742/profile/upload/2025/12/16/%E6%A3%80%E6%B5%8B%E6%8A%A5%E5%91%8A_20251216161249A001.pdf', '_blank');
      // sendpdfzy(row.opJczxBloodReportBaseId).then(response => {
      //   // 处理下载成功逻辑
      //   this.$message.success('报告下载成功');
      //   // 可以触发文件下载
      //   window.open(response.url, '_blank'); // 或使用其他下载方法
      // }).catch(error => {
      //   // 处理下载失败逻辑
    ...
    handlebgdy(row) {
      ...
      if (row.status == 0) {
        ...
        } else if (row.bloodTaskItemType == 4) {
          window.open(
            `/reportgenBVDV?bloodEntrustOrderId=${row.bloodEntrustOrderId}&status=${row.status}&bloodTaskItemType=${row.bloodTaskItemType}`,
            '_blank' // 新标签页
          )
        } else if (row.bloodTaskItemType == 3) {
          window.open(
            `/reportgenfjh?bloodEntrustOrderId=${row.bloodEntrustOrderId}&status=${row.status}&bloodTaskItemType=${row.bloodTaskItemType}`,
            '_blank' // 新标签页
          )
        }
      } else {
        ...
        if (row.bloodTaskItemType == 2) {
          ...
          window.open(
            `/reportgenBVDV?bloodEntrustOrderId=${row.bloodEntrustOrderId}&status=${row.status}&opJczxBloodReportBaseId=${row.opJczxBloodReportBaseId}&bloodTaskItemType=${row.bloodTaskItemType}`,
            '_blank' // 新标签页
          )
        } else if (row.bloodTaskItemType == 3) {
          window.open(
            `/reportgenfjh?bloodEntrustOrderId=${row.bloodEntrustOrderId}&status=${row.status}&opJczxBloodReportBaseId=${row.opJczxBloodReportBaseId}&bloodTaskItemType=${row.bloodTaskItemType}`,
            '_blank' // 新标签页
          )
        }
      }
    ...
    getList() {
      ...
      if (this.daterangeReportTime != null && this.daterangeReportTime != '') {
    ...
    resetQuery() {
      ...
      this.queryParams.bloodTaskItemTypeNotIn = '8,9' // 排除的检测项目类型
    ...
    handleSelectionChange(selection) {
      // 提取 bloodEntrustOrderId
      this.ids = selection.map(item => item.bloodEntrustOrderId)
    ...
    handleAddAll() {
      if (this.ids.length === 0) {
        this.$modal.msgWarning('请选择要编制的任务')
        return
      }

      // 构造参数
      const data = {
        bloodEntrustOrderIdList: this.ids
      }

      this.$modal.confirm('是否确认编制选中的 ' + this.ids.length + ' 条数据项？').then(() => {
        this.loading = true // 开启遮罩
        return addAll(data)
      }).then(() => {
        this.loading = false
        this.$modal.msgSuccess('编制成功')
        this.getList() // 刷新列表
        this.getTabCounts() // 刷新角标
      }).catch(() => {
        this.loading = false
      })
    },
    ...
    */
    ...
    getTabCounts() {
      ...
      // 如果 queryParams 里被清空了，这里强制补上默认值
      if (!countParams.bloodTaskItemType) {
        countParams.bloodTaskItemTypeNotIn = '8,9'
      }
      // --- 复用 getList() 中的日期处理逻辑 ---
