                    ...
                    <table class="detail-table">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in samples" :key="index">
                                ...
                                <td>{{ item.testMethod=='1'?'近红外':'化学法' }}</td>
                            </tr>
                        </tbody>
                    </table>
...
export default {
    dicts: ['requires_judgement', 'allows', 'report_receive_type', 'sample_return_policy', 'sample_packaging', 'storage_requirement', 'sample_state', 'testing_service_level'],
    data() {
        return {
            // 遮罩层
            loading: false,
    ...
    mounted() {
        ...
        if (printKey) {
            try {
                ...
                if (storageData) {
                    ...
                    // 开发环境注释掉下面这行，方便刷新调试；生产环境可以开启以节省空间
        ...
        // 3. 校验数据是否存在
        if (!src) {
            this.$alert('未找到打印数据，请关闭窗口重新点击打印。', '提示', {
        ...
        // 处理支付状态：确保是字符串 '0' 或 '1'，防止 radio 选中失效
        // 注意：如果 src.paymentStatus 是数字 0，(0 ? ... : ...) 会返回 false，所以要判断 undefined
        // this.paymentStatus = (src.paymentStatus !== null && src.paymentStatus !== undefined)
        //     ? String(src.paymentStatus)
        //     : '0';

        // this.paymentMethod = (src.paymentMethod !== null && src.paymentMethod !== undefined)
        //     ? String(src.paymentMethod)
        //     : '0';
