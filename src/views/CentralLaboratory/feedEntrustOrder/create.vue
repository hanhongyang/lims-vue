<template>
  <div class="app-container">
    <div class="entrust-form">
          ...
          <tr>
            ...
            <td width="35%">
              <div style="position: relative;">
                <el-autocomplete v-model="form.producerUnitName" :fetch-suggestions="querySearchAsync"
                  placeholder="请输入内容" @select="handleSelect" style="width:100%" :disabled="isZS"></el-autocomplete>

                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_producerUnitName']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_producerUnitName']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning"
                    style="color:#F56C6B;position:absolute;right:-10px;top:10px;cursor:pointer;"></i>
                </el-popover>
              </div>
            </td>
            <td width="15%">送检单位</td>
          ...
          <tr>
            ...
            <td>
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_reportMailingAddress']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_reportMailingAddress']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
                  ...
                  <i slot="reference" class="el-icon-warning"
                    style="color:#F56C6B;position:absolute;right:5px;top:10px;cursor:pointer;"></i>
                </el-popover>
              </div>
            </td>
            ...
            <td>
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_entrustContact']" placement="top" title="修改记录"
                  width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_entrustContact']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
                  ...
                  <i slot="reference" class="el-icon-warning"
                    style="color:#F56C6B;position:absolute;right:25px;top:10px;cursor:pointer;"></i>
                </el-popover>
              </div>
            </td>
          </tr>
          <tr>
            ...
            <td>
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_sendSampleDate']" placement="top" title="修改记录"
                  width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_sendSampleDate']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
            ...
            <td>
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_receiverName']" placement="top" title="修改记录"
                  width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_receiverName']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
          ...
          <tr>
            ...
            <td>
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_invoiceTitle']" placement="top" title="修改记录"
                  width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_invoiceTitle']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
            ...
            <td width="35%">
              <div style="display: flex; align-items: center; position: relative;">
                ...
                <el-popover
                  v-if="changeMap[form.opFeedEntrustOrderId + '_paymentStatus'] || changeMap[form.opFeedEntrustOrderId + '_paymentMethod']"
                  placement="top" title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <div v-if="changeMap[form.opFeedEntrustOrderId + '_paymentStatus']">
                      <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_paymentStatus']"
                        :key="'ps' + i" :timestamp="log.createTime">
                        <p><strong>付款状态</strong></p>
                        <p>旧: {{ log.oldValue == 1 ? '已付' : '未付' }} <br> 新: {{ log.newValue == 1 ? '已付' : '未付' }}</p>
                        <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                      </el-timeline-item>
                    </div>
                    <div v-if="changeMap[form.opFeedEntrustOrderId + '_paymentMethod']">
                      <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_paymentMethod']"
                        :key="'pm' + i" :timestamp="log.createTime">
                        <p><strong>付款方式</strong></p>
                        <p>旧: {{ log.oldValue == 1 ? '转账' : '现金' }} <br> 新: {{ log.newValue == 1 ? '转账' : '现金' }}</p>
                        <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                      </el-timeline-item>
                    </div>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning" style="color:#F56C6B;margin-left:5px;cursor:pointer;"></i>
                </el-popover>
              </div>
            </td>
          </tr>
          <tr>
            ...
            <td width="35%">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_invoiceType']" placement="top" title="修改记录"
                  width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_invoiceType']" :key="i"
                      :timestamp="log.createTime">
                      <p>旧: {{ getLogDisplayValue('invoiceType', log.oldValue) }} <br>
                        新: {{ getLogDisplayValue('invoiceType', log.newValue) }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning" style="color:#F56C6B;margin-left:5px;cursor:pointer;"></i>
            ...
            <td>
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_reportReceiver']" placement="top" title="修改记录"
                  width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_reportReceiver']"
                      :key="i" :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                      <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                    </el-timeline-item>
      ...
      <div class="form-section">
        <table class="info-table">
          <tr>
            ...
            <td width="35%">
              <div class="remarks">
                <p style="display: flex; align-items: center; margin: 8px 0; position: relative;">
                  ...
                  <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_requiresJudgement']" placement="top"
                    title="修改记录" width="300" trigger="hover">
                    <el-timeline>
                      <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_requiresJudgement']"
                        :key="i" :timestamp="log.createTime">
                        <p>旧: {{ getLogDisplayValue('requiresJudgement', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('requiresJudgement', log.newValue) }}</p>
                        <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                      </el-timeline-item>
                    </el-timeline>
                    <i slot="reference" class="el-icon-warning"
                      style="color:#F56C6B;margin-left:5px;cursor:pointer;"></i>
                  </el-popover>
                </p>
                <p style="display: flex; align-items: center; margin: 8px 0; position: relative;">
                  ...
                  <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_allowsSubcontracting']" placement="top"
                    title="修改记录" width="300" trigger="hover">
                    <el-timeline>
                      <el-timeline-item
                        v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_allowsSubcontracting']" :key="i"
                        :timestamp="log.createTime">
                        <p>旧: {{ getLogDisplayValue('allowsSubcontracting', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('allowsSubcontracting', log.newValue) }}</p>
                        <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                      </el-timeline-item>
                    </el-timeline>
                    <i slot="reference" class="el-icon-warning"
                ...
                <p style="display: flex; align-items: center; margin: 8px 0; position: relative;">
                  ...
                  <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_reportReceiveType']" placement="top"
                    title="修改记录" width="300" trigger="hover">
                    <el-timeline>
                      <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_reportReceiveType']"
                        :key="i" :timestamp="log.createTime">
                        <p>旧: {{ getLogDisplayValue('reportReceiveType', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('reportReceiveType', log.newValue) }}</p>
                        <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                      </el-timeline-item>
                    </el-timeline>
                    <i slot="reference" class="el-icon-warning"
            ...
            <td width="15%">样品要求</td>
            <td width="35%">
              <div class="sample-requirements" style="position: relative;">
                ...
                <el-popover v-if="changeMap[form.opFeedEntrustOrderId + '_sampleReturnPolicy']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, i) in changeMap[form.opFeedEntrustOrderId + '_sampleReturnPolicy']"
                      :key="i" :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ getLogDisplayValue('sampleReturnPolicy', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('sampleReturnPolicy', log.newValue) }}</p>
                        <div style="color:#999;font-size:12px">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning"
                    style="color:#F56C6B;position:absolute;right:0;top:0;cursor:pointer;"></i>
                </el-popover>
              </div>
            </td>
          </tr>
      ...
      <div class="form-section">
        <el-table :data="form.sampleList" border style="width: 100%">
          ...
          <el-table-column label="物料" width="160" align="center">
            <template slot-scope="scope">
              <div class="material-required" style="position: relative; min-height: 60px;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_materialName']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_materialName']" :key="idx"
                      :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p><strong>旧值:</strong> {{ log.oldValue }}</p>
                        <p><strong>新值:</strong> {{ log.newValue }}</p>
                        <div style="font-size: 12px; color: #999; margin-top:5px;">修改人: {{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning"
                    style="color: #F56C6B; position: absolute; right: 5px; top: 10px; cursor: pointer; z-index: 10;"></i>
                </el-popover>
          ...
          <el-table-column prop="name" label="类别" width="200" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_name']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_name']"
                      :key="idx" :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning"
          ...
          <el-table-column prop="model" label="样品数量" width="120" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_model']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_model']"
                      :key="idx" :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
          ...
          <el-table-column label="生产日期/批号" width="140" align="center" prop="batchNo">
            <template slot-scope="scope">
              <div style="position: relative;">
                <el-input v-model="scope.row.batchNo"></el-input>
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_batchNo']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_batchNo']"
                      :key="idx" :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
          ...
          <el-table-column prop="testItem" label="检验项目" width="240" align="center">
            <template slot-scope="scope">
              <div style="position: relative; padding-right: 20px;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_testItem']" placement="top"
                  title="修改记录" width="400" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_testItem']" :key="idx"
                      :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <div style="word-break: break-all;">
                          <p><strong>旧:</strong> {{ log.oldValue }}</p>
                          <p><strong>新:</strong> {{ log.newValue }}</p>
                        </div>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  ...
                  <i slot="reference" class="el-icon-warning"
                    style="color: #F56C6B; position: absolute; right: 0px; top: 10px; cursor: pointer; z-index:99;"></i>
                </el-popover>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="packaging" label="样品包装" width="110" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_packaging']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_packaging']" :key="idx"
                      :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ getLogDisplayValue('packaging', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('packaging', log.newValue) }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning"
                    style="color: #F56C6B; position: absolute; right: 0px; top: 0px; cursor: pointer;"></i>
                </el-popover>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="status" label="样品状况" width="180" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_status']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_status']"
                      :key="idx" :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ getLogDisplayValue('status', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('status', log.newValue) }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
          ...
          <el-table-column prop="storageRequirement" label="存储要求" width="100" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_storageRequirement']"
                  placement="top" title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_storageRequirement']"
                      :key="idx" :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ getLogDisplayValue('storageRequirement', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('storageRequirement', log.newValue) }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
          ...
          <el-table-column prop="testMethod" label="检验检测方法" width="200" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_testMethod']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_testMethod']" :key="idx"
                      :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ getLogDisplayValue('testMethod', log.oldValue) }} <br>
                          新: {{ getLogDisplayValue('testMethod', log.newValue) }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
                      </el-card>
                    </el-timeline-item>
                  </el-timeline>
          ...
          <el-table-column prop="model" label="样品来源" width="200" align="center" v-if="!isZS && isActiveTab !== '1'">
            <template slot-scope="scope">
              <div style="position: relative;">
                <el-input v-model="scope.row.materialSampleNo" placeholder="请选择样品来源" readonly>
                  <el-button slot="append" icon="el-icon-search"
                    @click="openSupplierDialogForSample(scope.$index)"></el-button>
                </el-input>
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_producerUnit']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_producerUnit']" :key="idx"
                      :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
          ...
          <!-- <el-table-column prop="model" label="备注" width="200" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                ...
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_sampleRemark']" placement="top"
                  title="修改记录" width="300" trigger="hover">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_sampleRemark']" :key="idx"
                      :timestamp="log.createTime">
                      <el-card shadow="never" body-style="padding: 10px">
                        <p>旧: {{ log.oldValue }} <br> 新: {{ log.newValue }}</p>
                        <div style="font-size: 12px; color: #999;">{{ log.createBy }}</div>
      ...
      <div v-if="deletedSampleList && deletedSampleList.length > 0"
        style="margin-top: 30px; border-top: 1px dashed #ccc; padding-top: 20px;">
        <div style="margin-bottom: 10px; color: #909399; font-weight: bold;">
          <i class="el-icon-delete-solid"></i> 已删除的历史样品
        </div>

        <el-table :data="deletedSampleList" border style="width: 100%" size="small"
          :row-style="{ background: '#f5f7fa', color: '#909399' }">
          <el-table-column label="样品编号" prop="sampleNo" align="center" width="120">
            <template slot-scope="scope"><span style="text-decoration: line-through;">{{ scope.row.sampleNo || '-'
            }}</span></template>
          </el-table-column>
          ...
          <el-table-column label="物料名称" prop="materialName" align="center">
            <template slot-scope="scope">
              <div style="position: relative;">
                {{ scope.row.materialName }}
                <el-popover v-if="changeMap[scope.row.opFeedEntrustOrderSampleId + '_materialName']" trigger="hover"
                  placement="top" width="300">
                  <el-timeline>
                    <el-timeline-item
                      v-for="(log, idx) in changeMap[scope.row.opFeedEntrustOrderSampleId + '_materialName']" :key="idx"
                      :timestamp="log.createTime">
                      <p>旧: {{ log.oldValue }} -> 新: {{ log.newValue }}</p>
                      <p style="font-size:12px;color:#ccc">{{ log.createBy }}</p>
                    </el-timeline-item>
                  </el-timeline>
                  <i slot="reference" class="el-icon-warning"
                    style="color: #909399; margin-left: 5px; cursor: pointer;"></i>
                </el-popover>
              </div>
            </template>
          </el-table-column>

          <el-table-column label="类别" prop="name" align="center" />
          <el-table-column label="规格" prop="model" align="center" />

          <el-table-column label="包装" prop="packaging" align="center">
            <template slot-scope="scope">{{ selectDictLabel(dict.type.sample_packaging, scope.row.packaging)
            }}</template>
          </el-table-column>

          <el-table-column label="状态" prop="status" align="center">
            <template slot-scope="scope">{{ selectDictLabel(dict.type.sample_state, scope.row.status) }}</template>
          </el-table-column>

          <el-table-column label="存储要求" prop="storageRequirement" align="center">
            <template slot-scope="scope">{{ selectDictLabel(dict.type.storage_requirement, scope.row.storageRequirement)
            }}</template>
          </el-table-column>

          <el-table-column label="检测方法" prop="testMethod" align="center">
            <template slot-scope="scope">{{ selectDictLabel(dict.type.feed_test_method, scope.row.testMethod)
            }}</template>
          </el-table-column>

          <el-table-column label="供应商" prop="producerUnit" align="center" />
          <el-table-column label="备注" prop="sampleRemark" align="center" />

          <el-table-column label="检测项目" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span v-if="scope.row.testItem">
                {{scope.row.testItem.map(i => i.itemName).join(', ')}}
              </span>
            </template>
          </el-table-column>

          <el-table-column label="操作" align="center" width="100">
            <template slot-scope="scope"><el-tag type="info" size="mini">已删除</el-tag></template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog title="选择样品来源" :visible.sync="supplierDialogVisible" width="60vw">
      <div class="dialog-container">
        <div class="selection-panel" style="width: 100%;">
          <!-- <el-input v-model="supplierQuery.name" placeholder="搜索样品来源" style="margin-bottom: 15px;">
          ...
          <el-table :data="supplierList" height="400" @row-dblclick="selectSupplier">
            <el-table-column prop="sampleNo" label="样品编码"></el-table-column>
            <el-table-column prop="invbillName" label="物料名称"></el-table-column>
            <el-table-column prop="invbillCode" label="物料编码"></el-table-column>
...
import {
  Reportsl, Reportslbc, zzhhx, zzhhxbc, jzcommit, savePdf, ckbg, receiveList
...
export default {
  ...
  data() {
    return {
      ...
      // ============ 新增状态字段 ============
      changeMap: {}, // 存储修改记录：Key=ID_Field, Value=Array
      deletedSampleList: [], // 存储已删除的样品
    };
  },
  watch: {
    'bsContactInfoList': {
  ...
  methods: {
    // 【新增】根据日志字段和值获取显示文本（处理字典）
    getLogDisplayValue(field, value) {
      if (!value) return '空';

      let dictType = '';
      switch (field) {
        case 'packaging': dictType = 'sample_packaging'; break;
        case 'status': dictType = 'sample_state'; break;
        case 'storageRequirement': dictType = 'storage_requirement'; break;
        case 'testMethod': dictType = 'feed_test_method'; break;
        case 'invoiceType': dictType = 'invoice_type'; break;
        case 'requiresJudgement': dictType = 'requires_judgement'; break;
        case 'allowsSubcontracting': dictType = 'allows'; break;
        case 'reportReceiveType': dictType = 'report_receive_type'; break;

        // 【新增】样品要求/返还政策
        case 'sampleReturnPolicy': dictType = 'sample_return_policy'; break;
        default: return value;
      }

      if (dictType && this.dict.type[dictType]) {
        const found = this.dict.type[dictType].find(d => d.value === String(value));
    ...
    // 获取生产企业列表---改为样品来源了
    getSupplierList() {
      receiveList(this.supplierQuery).then(response => {
    ...
    selectSupplier(row) {
      ...
      // }
      const sample = this.form.sampleList[this.currentSupplierIndex];
      this.$set(sample, 'materialSampleNo', row.sampleNo);
    ...
    loadData(id) {
      getFeedEntrustOrder(id).then(response => {
        ...
        // 1. 处理修改日志，转换为 Map 结构方便表格查找
        this.changeMap = {};
        if (this.form.changeLogs && this.form.changeLogs.length > 0) {
          this.form.changeLogs.forEach(log => {
            // 唯一Key规则: 样品ID + "_" + 字段名 (例如: 2448..._materialName)
            const key = `${log.businessId}_${log.fieldKey}`;
            if (!this.changeMap[key]) {
              this.changeMap[key] = [];
            }
            this.changeMap[key].push(log);
          });
        }

        // 2. 处理已删除样品
        this.deletedSampleList = this.form.deletedSampleList || [];
