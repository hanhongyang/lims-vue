  ...
  <div class="app-container">
          ...
          <el-col>
            <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
              ...
              <el-form-item label="账号" prop="userName">
                <el-input v-model="queryParams.userName" placeholder="请输入账号" clearable style="width: 240px"
            ...
            <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange" height="64vh">
              ...
              <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
                <template slot-scope="scope" v-if="scope.row.userId !== 1">
                  <el-button size="mini" type="text" icon="el-icon-edit" @click="handleQm(scope.row)"
                    v-hasPermi="['system:user:edit']">签名</el-button>
                  ...
                  <el-button size="mini" type="text" icon="el-icon-circle-check"
                    @click="handleCommand(handleAuthRole, scope.row)" v-hasPermi="['system:user:edit']">分配角色</el-button>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <!-- <p style="color: red;">密码必须是大小写+特殊符号+数字不能小于8位</p> -->
        <el-row>
          <el-col :span="12">
    ...
    <el-dialog title="签字板" :visible.sync="videoif" width="80%">
      <electronic @closeCanvas="closeCanvas" :showCanvas="videoif" @savecanva="savecanva" :SignatureImg="SignatureImg"
        ...
        :initialSignature="userSignatureUrl" />
    </el-dialog>
  </div>
</template>

<script>
import { listUser, getUser, delUser, addUser, updateUser, resetUserPwd, changeUserStatus, deptTreeSelect, updateUserSign } from "@/api/system/user";
import { getToken } from "@/utils/auth";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";
import { Splitpanes, Pane } from "splitpanes";
import "splitpanes/dist/splitpanes.css";
import electronic from '@/views/system/user/electronic.vue'
import { XZPic } from "@/api/hyd";
export default {
  name: "User",

  dicts: ['sys_normal_disable', 'sys_user_sex'],
  components: { Treeselect, Splitpanes, Pane, "electronic": electronic, },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    handleQm(row) {
      ...
      console.log(row, '888888888888888')
      this.videoif = true
      this.form = { ...row }
      // XZPic(this.$route.params.SjtaskInfo.testUserSignature).then((res) => {
      //   this.signTypes.检测人 = URL.createObjectURL(res)
      // })
      XZPic(row.sign).then((res) => {
        // this.userSignatureUrl = URL.createObjectURL(res)
        const reader = new FileReader()
        reader.onload = () => {
          this.userSignatureUrl = reader.result // base64
        }
        reader.readAsDataURL(res)
        // URL.createObjectURL(res)
      })
      // this.$set(this.form, 'sign', this.userSignatureUrl)
    ...
    onSignatureDone({ type, data, fileId, close }) {
      console.log(fileId, '111111111111')
      this.fileId = fileId
      // XZPic(this.fileId).then((res) => {
      ...
      //   // URL.createObjectURL(res)
      // })
      // this.$set(this.form, 'sign', fileId)
      console.log(this.form, '222222222')
      this.form.sign = fileId
      console.log(this.form, '222222222')
      if (this.form.userId && this.form.sign) {
        updateUserSign({ userId: this.form.userId, sign: this.form.sign }).then(res => {
          console.log(res)
          this.$modal.msgSuccess("修改成功");
          // this.open = false;
          this.getList();
        ...
        if (valid) {
          ...
          if (this.form.userId != undefined) {
            ...
            if (regex.test(this.form.password)) {
              ...
              this.$message.error("密码必须是大小写+特殊符号+数字不能小于8位");
            }
          }
