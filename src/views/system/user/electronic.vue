...
<script>
import { uploadAndGetUrl } from '@/api/common/common'
...
export default {
	...
	methods: {
		async loadSignatureImage() {
			if (!this.initialSignature) return
			...
			img.onload = () => {
				...
				this.signatureDataUrl = this.initialSignature
			}

			img.onerror = () => {
				console.warn('签名图片加载失败，留空画布')
			}
		...
		async uploadSignature() {
			...
			try {
				...
				// 使用封装的上传接口
				const response = await uploadAndGetUrl(formData)
