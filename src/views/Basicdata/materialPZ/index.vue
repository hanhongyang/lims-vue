<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="物料编号" prop="sapCode">
        <el-input v-model="queryParams.sapCode" placeholder="请输入物料编号" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="物料名称" prop="invbillName">
        <el-input v-model="queryParams.invbillName" placeholder="请输入物料名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="标签" prop="tag">
        <el-select v-model="queryParams.tag" placeholder="请选择标签" style="width: 100%">
          <el-option v-for="dict in dict.type.invbill_tag" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="类型" prop="isSapType">
        <el-select v-model="queryParams.isSapType" placeholder="请选择类型" style="width: 100%" clearable>
          <el-option label="sap物料" value="0" />
          <el-option label="手动新增物料" value="1" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <!--<el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-zoom-in" size="mini" @click="getAdd" :loading="btnLoad">新增
        </el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-zoom-in" size="mini" @click="getInfoAll" :loading="btnLoad">同步物料
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="invbillList" max-height="610px" style="width: 100%;">
      ...
      <el-table-column label="物料编号" align="center" prop="sapCode" width="200" />
      <el-table-column label="物料名称" align="center" prop="invbillName" min-width="220" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="125px">
        <el-form-item label="物料编号" prop="sapCode">
          <el-input v-model="form.sapCode" placeholder="请输入物料编号" :disabled="true" />
        </el-form-item>
        <el-form-item label="物料名称" prop="invbillName">
          <el-input v-model="form.invbillName" placeholder="请输入名称" :disabled="title != '新增物料'" />
        </el-form-item>
        <el-form-item label="标签" prop="tag">
        ...
        <el-form-item label="样品状况" prop="sampleState" v-if="form.tag == '1'">
          <el-select v-model="form.sampleState" placeholder="请选择标签" style="width: 100%">
            <el-option v-for="dict in dict.type.sample_state" :key="dict.value" :label="dict.label"
              :value="dict.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="类型" prop="materialType">
          <el-select v-model="form.materialType" filterable placeholder="请选择类型" style="width: 100%">
...
import { getinvbilllist, getsyncBaseInvbill, getinvbill, updateInvbill, addInvbill } from '@/api/Basicdata/invbill'
import { listDept } from "@/api/system/dept";

export default {
  name: 'materialindex',
  dicts: ['sys_yes_no2', 'invbill_tag', 'sample_state', 'material_type'],
  data() {
    return {
      // 部门选择相关数据
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.title === '新增物料') {
            addInvbill(submitData).then(response => {
