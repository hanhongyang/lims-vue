<template>
    <div class="app-container">
        ...
        <el-table ref="multipleTable" v-loading="loading" :data="sampleReceiveList" height="70%" :row-key="getRowKeys">
            ...
            <el-table-column label="装奶时间" align="center" prop="milkFillingTime" />
            <el-table-column label="最早一滴挤奶时间" align="center" prop="firstMilkingTime" width="180" />
            <el-table-column label="装奶量" align="center" prop="totalMilkCapacity" />
...
<script>
import { listmilkFilling } from '@/api/milkSource/MilkLoadingList'
export default {
    name: 'MilkTanker',
    dicts: [],
    data() {
    ...
    methods: {
        ...
        getList() {
            ...
            listmilkFilling(this.queryParams).then(res => {
        ...
        /** 修改按钮操作 */
        handleUpdate(row) {
            const opMilkFillingOrderId = row.opMilkFillingOrderId
            this.$router.push({ path: '/milkSource/MilkLoadingListcreate', query: { opMilkFillingOrderId: opMilkFillingOrderId } });
        },
        handleExport() {
            this.download('milkFilling/order/export', {
                ...this.queryParams
            }, `装奶单_${this.activeTab}_${new Date().getTime()}.xlsx`)
        },
    }
}
</script>