      ...
      <div class="form-section">
        <table class="info-table">
          ...
          <tr>
            ...
            <td>样品类型</td>
            <td>
              <el-radio-group v-model="form.samplingType">
                <el-radio label="1">库存</el-radio>
                <el-radio label="2">垫料</el-radio>
              </el-radio-group>
            </td>
          </tr>
          <tr>
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="样品编号" prop="sampleNo">
                <el-input v-model="form.sampleNo" placeholder="自动生成" disabled />
              </el-form-item>
            </el-col>
...
export default {
  ...
  methods: {
    ...
    handleCarReportPreview() {
      if (!this.previewUrlList.length) {
        this.$message.warning('暂无检测报告')
        return
      }
    ...
    loadData(id) {
      sampleInfo({
        ...
        const urlStr = response.data.fileInfoUrl || ''
        this.previewUrlList = Array.isArray(urlStr)
          ? urlStr
          : urlStr.split(',').filter(Boolean)
        // 关键保护
