...
function sheet_from_array_of_arrays(data, headerNum, editHeaders) {
  ...
  for (let R = 0; R !== data.length; ++R) {
    ...
    for (let C = 0; C !== data[R].length; ++C) {
      ...
      ws[cell_ref] = cell
      // 判断当前格是表头还是数据并添加样式
      ws[cell_ref].s = R <= headerNum - 1 ? _.cloneDeep(titleRef) : dataRef
      if (editHeaders && editHeaders.length > 0 && _.find(editHeaders, t => t === data[R][C]) && R <= headerNum - 1) {
...
export function export_json_to_excel({
  ...
  const wb = new Workbook(),
    ws = sheet_from_array_of_arrays(data, headerNum, editHeaders)
  if (autoWidth) {
    /* 设置worksheet每列的最大宽度 */
    const maxWidth = 20
